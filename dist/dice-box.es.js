var Qb = Object.defineProperty;
var Pb = (Z, X, u) => X in Z ? Qb(Z, X, { enumerable: !0, configurable: !0, writable: !0, value: u }) : Z[X] = u;
var Wl = (Z, X, u) => (Pb(Z, typeof X != "symbol" ? X + "" : X, u), u), Vd = (Z, X, u) => {
  if (!X.has(Z))
    throw TypeError("Cannot " + u);
};
var F = (Z, X, u) => (Vd(Z, X, "read from private field"), u ? u.call(Z) : X.get(Z)), j = (Z, X, u) => {
  if (X.has(Z))
    throw TypeError("Cannot add the same private member more than once");
  X instanceof WeakSet ? X.add(Z) : X.set(Z, u);
}, D = (Z, X, u, G) => (Vd(Z, X, "write to private field"), G ? G.call(Z, u) : X.set(Z, u), u);
var nl = (Z, X, u, G) => ({
  set _(s) {
    D(Z, X, s, u);
  },
  get _() {
    return F(Z, X, G);
  }
}), $ = (Z, X, u) => (Vd(Z, X, "access private method"), u);
function gb(Z, X) {
  for (var u = 0; u < X.length; u++) {
    const G = X[u];
    if (typeof G != "string" && !Array.isArray(G)) {
      for (const s in G)
        if (s !== "default" && !(s in Z)) {
          const e = Object.getOwnPropertyDescriptor(G, s);
          e && Object.defineProperty(Z, s, e.get ? e : {
            enumerable: !0,
            get: () => G[s]
          });
        }
    }
  }
  return Object.freeze(Object.defineProperty(Z, Symbol.toStringTag, { value: "Module" }));
}
function jb(Z) {
  const { selector: X, id: u } = Z;
  let G = document.body, s = document.createElement("canvas");
  if (s.id = u, s.classList.add("dice-box-canvas"), X) {
    if (typeof X != "string")
      throw new Error("You must provide a DOM selector as the first argument in order to render the Dice Box");
    if (G = document.querySelector(X), !(G != null && G.nodeName))
      throw new Error(`DiceBox target DOM node: '${X}' not found or not available yet. Try invoking inside a DOMContentLoaded event`);
  }
  return G.appendChild(s), s;
}
const Wb = "", db = typeof window < "u" && window.Blob && new Blob([atob(Wb)], { type: "text/javascript;charset=utf-8" });
function Eb() {
  let Z;
  try {
    if (Z = db && (window.URL || window.webkitURL).createObjectURL(db), !Z)
      throw "";
    return new Worker(Z);
  } catch {
    return new Worker("data:application/javascript;base64," + Wb);
  } finally {
    Z && (window.URL || window.webkitURL).revokeObjectURL(Z);
  }
}
const Ib = (Z) => {
  let X;
  return function() {
    let u = this, G = arguments;
    X && window.cancelAnimationFrame(X), X = window.requestAnimationFrame(function() {
      Z.apply(u, G);
    });
  };
}, Tb = (Z = { dedupe: !1 }) => {
  const { dedupe: X } = Z;
  let u = [], G;
  const s = (d) => (X && (u = []), u.push(d), G || (G = e()), G.finally(() => {
    G = void 0;
  })), e = async () => {
    const d = [];
    for (; u.length; ) {
      const Y = u.shift();
      d.push(await Y());
    }
    return d;
  };
  return { push: s, queue: u, flush: () => G || Promise.resolve([]) };
}, yX = (Z) => JSON.parse(JSON.stringify(Z));
class bb {
  /**
   * Generate a random number between 0 (inclusive) and 1 (exclusive).
   * A drop in replacement for Math.random()
   * @return {number}
   */
  static value() {
    const X = window.crypto || window.msCrypto, u = new Uint32Array(1);
    return X.getRandomValues(u)[0] / 2 ** 32;
  }
  /**
   * Generate a very good random number between min (inclusive) and max (exclusive) by using crypto.getRandomValues() twice.
   * @param  {number} min
   * @param  {number} max
   * @return {number}
   */
  static range(X, u) {
    return Math.floor(Math.pow(10, 14) * this.value() * this.value()) % (u - X + 1) + X;
  }
}
const Mb = (Z) => {
  let X = !1, u = Z.slice(Z.startsWith("#") ? 1 : 0);
  u.length === 3 ? u = [...u].map((s) => s + s).join("") : u.length === 8 && (X = !0), u = parseInt(u, 16);
  let G = {
    r: u >>> 16,
    g: (u & 65280) >>> 8,
    b: u & 255
  };
  return X && (G.r = u >>> 24, G.g = (u & 16711680) >>> 16, G.b = (u & 65280) >>> 8, G.a = u & 255), G;
};
function Db() {
  try {
    const Z = document.createElement("canvas");
    return !!window.WebGLRenderingContext && (Z.getContext("webgl") || Z.getContext("experimental-webgl"));
  } catch {
    return !1;
  }
}
var Ab = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function Ob(Z) {
  return Z && Z.__esModule && Object.prototype.hasOwnProperty.call(Z, "default") ? Z.default : Z;
}
var ib = { exports: {} };
(function(Z, X) {
  (function(u, G) {
    Z.exports = G();
  })(Ab, () => (() => {
    var u = { 95: (s, e, d) => {
      Object.defineProperty(e, "__esModule", { value: !0 }), e.DiceRoller = void 0;
      const Y = d(51);
      e.DiceRoller = class {
        constructor(c, l = 1e3) {
          this.randFunction = Math.random, this.maxRollCount = 1e3, c && (this.randFunction = c), this.maxRollCount = l;
        }
        parse(c) {
          return Y.parse(c);
        }
        roll(c) {
          const l = Y.parse(c);
          return this.rollType(l);
        }
        rollValue(c) {
          return this.roll(c).value;
        }
        rollParsed(c) {
          return this.rollType(c);
        }
        rollType(c) {
          let l;
          switch (c.type) {
            case "diceExpression":
              l = this.rollDiceExpr(c);
              break;
            case "group":
              l = this.rollGroup(c);
              break;
            case "die":
              l = this.rollDie(c);
              break;
            case "expression":
              l = this.rollExpression(c);
              break;
            case "mathfunction":
              l = this.rollFunction(c);
              break;
            case "inline":
              l = this.rollType(c.expr);
              break;
            case "number":
              l = Object.assign(Object.assign({}, c), { success: null, successes: 0, failures: 0, valid: !0, order: 0 });
              break;
            default:
              throw new Error(`Unable to render ${c.type}`);
          }
          return c.label && (l.label = c.label), l;
        }
        rollDiceExpr(c) {
          const l = this.rollType(c.head), n = [l], V = [], a = c.ops.reduce((t, L, N) => {
            const J = this.rollType(L.tail);
            switch (J.order = N, n.push(J), V.push(L.op), L.op) {
              case "+":
                return t + J.value;
              case "-":
                return t - J.value;
              default:
                return t;
            }
          }, l.value);
          return { dice: n, ops: V, success: null, successes: 0, failures: 0, type: "diceexpressionroll", valid: !0, value: a, order: 0 };
        }
        rollGroup(c) {
          let l = c.rolls.map((L, N) => Object.assign(Object.assign({}, this.rollType(L)), { order: N })), n = 0, V = 0, a = !1;
          if (c.mods) {
            const L = c.mods, N = (J) => (a = L.some((y) => ["failure", "success"].includes(y.type)), J = L.reduce((y, C) => this.applyGroupMod(y, C), J), a && (J = J.map((y) => (n += y.successes, V += y.failures, y.value = y.successes - y.failures, y.success = y.value > 0, y))), J);
            if (l.length === 1 && ["die", "diceexpressionroll"].includes(l[0].type)) {
              const J = l[0];
              let y = J.type === "die" ? J.rolls : J.dice.filter((C) => C.type !== "number").reduce((C, T) => [...C, ...T.type === "die" ? T.rolls : T.dice], []);
              y = N(y), J.value = y.reduce((C, T) => T.valid ? C + T.value : C, 0);
            } else
              l = N(l);
          }
          const t = l.reduce((L, N) => N.valid ? L + N.value : L, 0);
          return { dice: l, success: a ? t > 0 : null, successes: n, failures: V, type: "grouproll", valid: !0, value: t, order: 0 };
        }
        rollDie(c) {
          const l = this.rollType(c.count);
          if (l.value > this.maxRollCount)
            throw new Error("Entered number of dice too large.");
          let n, V;
          c.die.type === "fate" ? (V = { type: "fate", success: null, successes: 0, failures: 0, valid: !1, value: 0, order: 0 }, n = Array.from({ length: l.value }, (y, C) => this.generateFateRoll(C))) : (V = this.rollType(c.die), n = Array.from({ length: l.value }, (y, C) => this.generateDiceRoll(V.value, C))), c.mods && (n = c.mods.reduce((y, C) => this.applyMod(y, C), n));
          let a = 0, t = 0;
          c.targets && (n = c.targets.reduce((y, C) => this.applyMod(y, C), n).map((y) => (a += y.successes, t += y.failures, y.value = y.successes - y.failures, y.success = y.value > 0, y)));
          let L = !1, N = 0;
          if (c.match) {
            const y = c.match, C = n.reduce((E, ll) => E.set(ll.roll, (E.get(ll.roll) || 0) + 1), /* @__PURE__ */ new Map()), T = new Set(Array.from(C.entries()).filter(([E, ll]) => ll >= y.min.value).filter(([E]) => !(y.mod && y.expr) || this.successTest(y.mod, this.rollType(y.expr).value, E)).map(([E]) => E));
            n.filter((E) => T.has(E.roll)).forEach((E) => E.matched = !0), y.count && (L = !0, N = T.size);
          }
          c.sort && (n = this.applySort(n, c.sort));
          const J = n.reduce((y, C) => C.valid ? y + C.value : y, 0);
          return { count: l, die: V, rolls: n, success: c.targets ? J > 0 : null, successes: a, failures: t, type: "die", valid: !0, value: L ? N : J, order: 0, matched: L };
        }
        rollExpression(c) {
          const l = this.rollType(c.head), n = [l], V = [], a = c.ops.reduce((t, L) => {
            const N = this.rollType(L.tail);
            switch (n.push(N), V.push(L.op), L.op) {
              case "+":
                return t + N.value;
              case "-":
                return t - N.value;
              case "*":
                return t * N.value;
              case "/":
                return t / N.value;
              case "%":
                return t % N.value;
              case "**":
                return t ** N.value;
              default:
                return t;
            }
          }, l.value);
          return { dice: n, ops: V, success: null, successes: 0, failures: 0, type: "expressionroll", valid: !0, value: a, order: 0 };
        }
        rollFunction(c) {
          const l = this.rollType(c.expr);
          let n;
          switch (c.op) {
            case "floor":
              n = Math.floor(l.value);
              break;
            case "ceil":
              n = Math.ceil(l.value);
              break;
            case "round":
              n = Math.round(l.value);
              break;
            case "abs":
              n = Math.abs(l.value);
              break;
            default:
              n = l.value;
          }
          return { expr: l, op: c.op, success: null, successes: 0, failures: 0, type: "mathfunction", valid: !0, value: n, order: 0 };
        }
        applyGroupMod(c, l) {
          return this.getGroupModMethod(l)(c);
        }
        getGroupModMethod(c) {
          const l = (n) => n.value;
          switch (c.type) {
            case "success":
              return this.getSuccessMethod(c, l);
            case "failure":
              return this.getFailureMethod(c, l);
            case "keep":
              return this.getKeepMethod(c, l);
            case "drop":
              return this.getDropMethod(c, l);
            default:
              throw new Error(`Mod ${c.type} is not recognised`);
          }
        }
        applyMod(c, l) {
          return this.getModMethod(l)(c);
        }
        getModMethod(c) {
          const l = (n) => n.roll;
          switch (c.type) {
            case "success":
              return this.getSuccessMethod(c, l);
            case "failure":
              return this.getFailureMethod(c, l);
            case "crit":
              return this.getCritSuccessMethod(c, l);
            case "critfail":
              return this.getCritFailureMethod(c, l);
            case "keep":
              return (n) => this.getKeepMethod(c, l)(n).sort((V, a) => V.order - a.order);
            case "drop":
              return (n) => this.getDropMethod(c, l)(n).sort((V, a) => V.order - a.order);
            case "explode":
              return this.getExplodeMethod(c);
            case "compound":
              return this.getCompoundMethod(c);
            case "penetrate":
              return this.getPenetrateMethod(c);
            case "reroll":
              return this.getReRollMethod(c);
            case "rerollOnce":
              return this.getReRollOnceMethod(c);
            default:
              throw new Error(`Mod ${c.type} is not recognised`);
          }
        }
        applySort(c, l) {
          return c.sort((n, V) => l.asc ? n.roll - V.roll : V.roll - n.roll), c.forEach((n, V) => n.order = V), c;
        }
        getCritSuccessMethod(c, l) {
          const n = this.rollType(c.expr);
          return (V) => V.map((a) => {
            if (!a.valid || a.type !== "roll" || a.success)
              return a;
            const t = a;
            return this.successTest(c.mod, n.value, l(a)) ? t.critical = "success" : t.critical === "success" && (t.critical = null), a;
          });
        }
        getCritFailureMethod(c, l) {
          const n = this.rollType(c.expr);
          return (V) => V.map((a) => {
            if (!a.valid || a.type !== "roll" || a.success)
              return a;
            const t = a;
            return this.successTest(c.mod, n.value, l(a)) ? t.critical = "failure" : t.critical === "failure" && (t.critical = null), a;
          });
        }
        getSuccessMethod(c, l) {
          const n = this.rollType(c.expr);
          return (V) => V.map((a) => (a.valid && this.successTest(c.mod, n.value, l(a)) && (a.successes += 1), a));
        }
        getFailureMethod(c, l) {
          const n = this.rollType(c.expr);
          return (V) => V.map((a) => (a.valid && this.successTest(c.mod, n.value, l(a)) && (a.failures += 1), a));
        }
        getKeepMethod(c, l) {
          const n = this.rollType(c.expr);
          return (V) => {
            if (V.length === 0)
              return V;
            V = V.sort((J, y) => c.highlow === "l" ? l(y) - l(J) : l(J) - l(y)).sort((J, y) => (J.valid ? 1 : 0) - (y.valid ? 1 : 0));
            const a = Math.max(Math.min(n.value, V.length), 0);
            let t = 0, L = 0;
            const N = V.reduce((J, y) => (y.valid ? 1 : 0) + J, 0) - a;
            for (; L < V.length && t < N; )
              V[L].valid && (V[L].valid = !1, V[L].drop = !0, t++), L++;
            return V;
          };
        }
        getDropMethod(c, l) {
          const n = this.rollType(c.expr);
          return (V) => {
            V = V.sort((N, J) => c.highlow === "h" ? l(J) - l(N) : l(N) - l(J));
            const a = Math.max(Math.min(n.value, V.length), 0);
            let t = 0, L = 0;
            for (; L < V.length && t < a; )
              V[L].valid && (V[L].valid = !1, V[L].drop = !0, t++), L++;
            return V;
          };
        }
        getExplodeMethod(c) {
          const l = c.target ? this.rollType(c.target.value) : null;
          return (n) => {
            const V = l ? (a) => this.successTest(c.target.mod, l.value, a.roll) : (a) => this.successTest("=", a.type === "fateroll" ? 1 : a.die, a.roll);
            if (n[0].type === "roll" && V({ roll: 1 }) && V({ roll: n[0].die }))
              throw new Error("Invalid reroll target");
            for (let a = 0; a < n.length; a++) {
              let t = n[a];
              t.order = a;
              let L = 0;
              for (; V(t) && L++ < 1e3; ) {
                t.explode = !0;
                const N = this.reRoll(t, ++a);
                n.splice(a, 0, N), t = N;
              }
            }
            return n;
          };
        }
        getCompoundMethod(c) {
          const l = c.target ? this.rollType(c.target.value) : null;
          return (n) => {
            const V = l ? (a) => this.successTest(c.target.mod, l.value, a.roll) : (a) => this.successTest("=", a.type === "fateroll" ? 1 : a.die, a.roll);
            if (n[0].type === "roll" && V({ roll: 1 }) && V({ roll: n[0].die }))
              throw new Error("Invalid reroll target");
            for (let a = 0; a < n.length; a++) {
              let t = n[a], L = t.roll, N = 0;
              for (; V(t) && N++ < 1e3; ) {
                t.explode = !0;
                const J = this.reRoll(t, a + 1);
                L += J.roll, t = J;
              }
              n[a].value = L, n[a].roll = L;
            }
            return n;
          };
        }
        getPenetrateMethod(c) {
          const l = c.target ? this.rollType(c.target.value) : null;
          return (n) => {
            const V = l ? (a) => this.successTest(c.target.mod, l.value, a.roll) : (a) => this.successTest("=", a.type === "fateroll" ? 1 : a.die, a.roll);
            if (l && n[0].type === "roll" && V(n[0]) && this.successTest(c.target.mod, l.value, 1))
              throw new Error("Invalid reroll target");
            for (let a = 0; a < n.length; a++) {
              let t = n[a];
              t.order = a;
              let L = 0;
              for (; V(t) && L++ < 1e3; ) {
                t.explode = !0;
                const N = this.reRoll(t, ++a);
                N.value -= 1, n.splice(a, 0, N), t = N;
              }
            }
            return n;
          };
        }
        getReRollMethod(c) {
          const l = c.target ? this.successTest.bind(null, c.target.mod, this.rollType(c.target.value).value) : this.successTest.bind(null, "=", 1);
          return (n) => {
            if (n[0].type === "roll" && l(1) && l(n[0].die))
              throw new Error("Invalid reroll target");
            for (let V = 0; V < n.length; V++)
              for (; l(n[V].roll); ) {
                n[V].reroll = !0, n[V].valid = !1;
                const a = this.reRoll(n[V], V + 1);
                n.splice(++V, 0, a);
              }
            return n;
          };
        }
        getReRollOnceMethod(c) {
          const l = c.target ? this.successTest.bind(null, c.target.mod, this.rollType(c.target.value).value) : this.successTest.bind(null, "=", 1);
          return (n) => {
            if (n[0].type === "roll" && l(1) && l(n[0].die))
              throw new Error("Invalid reroll target");
            for (let V = 0; V < n.length; V++)
              if (l(n[V].roll)) {
                n[V].reroll = !0, n[V].valid = !1;
                const a = this.reRoll(n[V], V + 1);
                n.splice(++V, 0, a);
              }
            return n;
          };
        }
        successTest(c, l, n) {
          switch (c) {
            case ">":
              return n >= l;
            case "<":
              return n <= l;
            default:
              return n == l;
          }
        }
        reRoll(c, l) {
          switch (c.type) {
            case "roll":
              return this.generateDiceRoll(c.die, l);
            case "fateroll":
              return this.generateFateRoll(l);
            default:
              throw new Error(`Cannot do a reroll of a ${c.type}.`);
          }
        }
        generateDiceRoll(c, l) {
          const n = parseInt((this.randFunction() * c).toFixed(), 10) + 1;
          return { critical: n === c ? "success" : n === 1 ? "failure" : null, die: c, matched: !1, order: l, roll: n, success: null, successes: 0, failures: 0, type: "roll", valid: !0, value: n };
        }
        generateFateRoll(c) {
          const l = Math.floor(3 * this.randFunction()) - 1;
          return { matched: !1, order: c, roll: l, success: null, successes: 0, failures: 0, type: "fateroll", valid: !0, value: l };
        }
      };
    }, 619: (s, e) => {
      Object.defineProperty(e, "__esModule", { value: !0 }), e.DiscordRollRenderer = void 0, e.DiscordRollRenderer = class {
        render(d) {
          return this.doRender(d, !0);
        }
        doRender(d, Y = !1) {
          let c = "";
          switch (d.type) {
            case "diceexpressionroll":
              c = this.renderGroupExpr(d);
              break;
            case "grouproll":
              c = this.renderGroup(d);
              break;
            case "die":
              c = this.renderDie(d);
              break;
            case "expressionroll":
              c = this.renderExpression(d);
              break;
            case "mathfunction":
              c = this.renderFunction(d);
              break;
            case "roll":
              return this.renderRoll(d);
            case "fateroll":
              return this.renderFateRoll(d);
            case "number":
              const l = d.label ? ` (${d.label})` : "";
              return `${d.value}${l}`;
            case "fate":
              return "F";
            default:
              throw new Error("Unable to render");
          }
          return d.valid || (c = "~~" + c.replace(/~~/g, "") + "~~"), Y ? this.stripBrackets(c) : d.label ? `(${d.label}: ${c})` : c;
        }
        renderGroup(d) {
          const Y = [];
          for (const c of d.dice)
            Y.push(this.doRender(c));
          return Y.length > 1 ? `{ ${Y.join(" + ")} } = ${d.value}` : `{ ${this.stripBrackets(Y[0])} } = ${d.value}`;
        }
        renderGroupExpr(d) {
          const Y = [];
          for (const c of d.dice)
            Y.push(this.doRender(c));
          return Y.length > 1 ? `(${Y.join(" + ")} = ${d.value})` : Y[0];
        }
        renderDie(d) {
          const Y = [];
          for (const n of d.rolls)
            Y.push(this.doRender(n));
          let c = `${Y.join(", ")}`;
          ["number", "fate"].includes(d.die.type) && d.count.type === "number" || (c += `[*Rolling: ${this.doRender(d.count)}d${this.doRender(d.die)}*]`);
          const l = d.matched ? " Match" + (d.value === 1 ? "" : "es") : "";
          return c += ` = ${d.value}${l}`, `(${c})`;
        }
        renderExpression(d) {
          if (d.dice.length > 1) {
            const Y = [];
            for (let c = 0; c < d.dice.length - 1; c++)
              Y.push(this.doRender(d.dice[c])), Y.push(d.ops[c]);
            return Y.push(this.doRender(d.dice.slice(-1)[0])), Y.push("="), Y.push(d.value + ""), `(${Y.join(" ")})`;
          }
          return d.dice[0].type === "number" ? d.value + "" : this.doRender(d.dice[0]);
        }
        renderFunction(d) {
          const Y = this.doRender(d.expr);
          return `(${d.op}${this.addBrackets(Y)} = ${d.value})`;
        }
        addBrackets(d) {
          return d.startsWith("(") || (d = `(${d}`), d.endsWith(")") || (d = `${d})`), d;
        }
        stripBrackets(d) {
          return d.startsWith("(") && (d = d.substring(1)), d.endsWith(")") && (d = d.substring(0, d.length - 1)), d;
        }
        renderRoll(d) {
          let Y = `${d.roll}`;
          return d.valid ? d.success && d.value === 1 ? Y = `**${d.roll}**` : d.success && d.value === -1 ? Y = `*${d.roll}*` : d.success || d.critical !== "success" ? d.success || d.critical !== "failure" || (Y = `*${d.roll}*`) : Y = `**${d.roll}**` : Y = `~~${d.roll}~~`, d.matched && (Y = `__${Y}__`), Y;
        }
        renderFateRoll(d) {
          const Y = d.roll === 0 ? "0" : d.roll > 0 ? "+" : "-";
          let c = `${d.roll}`;
          return d.valid ? d.success && d.value === 1 ? c = `**${Y}**` : d.success && d.value === -1 && (c = `*${Y}*`) : c = `~~${Y}~~`, d.matched && (c = `__${c}__`), c;
        }
      };
    }, 607: function(s, e, d) {
      var Y = this && this.__createBinding || (Object.create ? function(l, n, V, a) {
        a === void 0 && (a = V);
        var t = Object.getOwnPropertyDescriptor(n, V);
        t && !("get" in t ? !n.__esModule : t.writable || t.configurable) || (t = { enumerable: !0, get: function() {
          return n[V];
        } }), Object.defineProperty(l, a, t);
      } : function(l, n, V, a) {
        a === void 0 && (a = V), l[a] = n[V];
      }), c = this && this.__exportStar || function(l, n) {
        for (var V in l)
          V === "default" || Object.prototype.hasOwnProperty.call(n, V) || Y(n, l, V);
      };
      Object.defineProperty(e, "__esModule", { value: !0 }), c(d(95), e), c(d(604), e), c(d(234), e), c(d(619), e), c(d(54), e);
    }, 604: (s, e) => {
      Object.defineProperty(e, "__esModule", { value: !0 });
    }, 234: (s, e) => {
      Object.defineProperty(e, "__esModule", { value: !0 });
    }, 54: (s, e) => {
      Object.defineProperty(e, "__esModule", { value: !0 });
    }, 51: (s) => {
      function e(d, Y, c, l) {
        this.message = d, this.expected = Y, this.found = c, this.location = l, this.name = "SyntaxError", typeof Error.captureStackTrace == "function" && Error.captureStackTrace(this, e);
      }
      (function(d, Y) {
        function c() {
          this.constructor = d;
        }
        c.prototype = Y.prototype, d.prototype = new c();
      })(e, Error), e.buildMessage = function(d, Y) {
        var c = { literal: function(a) {
          return '"' + n(a.text) + '"';
        }, class: function(a) {
          var t, L = "";
          for (t = 0; t < a.parts.length; t++)
            L += a.parts[t] instanceof Array ? V(a.parts[t][0]) + "-" + V(a.parts[t][1]) : V(a.parts[t]);
          return "[" + (a.inverted ? "^" : "") + L + "]";
        }, any: function(a) {
          return "any character";
        }, end: function(a) {
          return "end of input";
        }, other: function(a) {
          return a.description;
        } };
        function l(a) {
          return a.charCodeAt(0).toString(16).toUpperCase();
        }
        function n(a) {
          return a.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/\0/g, "\\0").replace(/\t/g, "\\t").replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/[\x00-\x0F]/g, function(t) {
            return "\\x0" + l(t);
          }).replace(/[\x10-\x1F\x7F-\x9F]/g, function(t) {
            return "\\x" + l(t);
          });
        }
        function V(a) {
          return a.replace(/\\/g, "\\\\").replace(/\]/g, "\\]").replace(/\^/g, "\\^").replace(/-/g, "\\-").replace(/\0/g, "\\0").replace(/\t/g, "\\t").replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/[\x00-\x0F]/g, function(t) {
            return "\\x0" + l(t);
          }).replace(/[\x10-\x1F\x7F-\x9F]/g, function(t) {
            return "\\x" + l(t);
          });
        }
        return "Expected " + function(a) {
          var t, L, N, J = new Array(a.length);
          for (t = 0; t < a.length; t++)
            J[t] = (N = a[t], c[N.type](N));
          if (J.sort(), J.length > 0) {
            for (t = 1, L = 1; t < J.length; t++)
              J[t - 1] !== J[t] && (J[L] = J[t], L++);
            J.length = L;
          }
          switch (J.length) {
            case 1:
              return J[0];
            case 2:
              return J[0] + " or " + J[1];
            default:
              return J.slice(0, -1).join(", ") + ", or " + J[J.length - 1];
          }
        }(d) + " but " + function(a) {
          return a ? '"' + n(a) + '"' : "end of input";
        }(Y) + " found.";
      }, s.exports = { SyntaxError: e, parse: function(d, Y) {
        Y = Y !== void 0 ? Y : {};
        var c, l = {}, n = { start: jd }, V = jd, a = { type: "any" }, t = r("[[", !1), L = r("]]", !1), N = function(m, W) {
          return W && (m.label = W), m;
        }, J = ">", y = r(">", !1), C = "<", T = r("<", !1), E = "=", ll = r("=", !1), ol = r("f", !1), xl = r("cs", !1), Kl = r("cf", !1), Cl = r("m", !1), wl = r("t", !1), ul = r("k", !1), zl = r("l", !1), Xl = r("h", !1), A = r("d", !1), bl = r("{", !1), Gl = r(",", !1), Jb = r("}", !1), Bl = "+", Ul = r("+", !1), yb = r("s", !1), fb = r("a", !1), vb = r("!", !1), Lb = r("!!", !1), Nb = r("!p", !1), Sb = r("r", !1), Fb = r("ro", !1), rb = r("F", !1), _l = r("%", !1), Ld = r("(", !1), Nd = r(")", !1), ld = r("-", !1), dd = function(m, W) {
          return W.length == 0 ? m : { head: m, type: "expression", ops: W.map((i) => ({ type: "math", op: i[1], tail: i[3] })) };
        }, Sd = r("*", !1), Fd = r("/", !1), kl = "**", rd = r("**", !1), Hd = "floor", Hb = r("floor", !1), xd = "ceil", xb = r("ceil", !1), Kd = "round", Kb = r("round", !1), Cb = r("abs", !1), wb = Qd("integer"), Cd = /^[0-9]/, wd = Xd([["0", "9"]], !1, !1), zb = r("[", !1), zd = /^[^\]]/, Bd = Xd(["]"], !0, !1), Bb = r("]", !1), Ub = Qd("whitespace"), Ud = /^[ \t\n\r]/, kd = Xd([" ", "	", `
`, "\r"], !1, !1), b = 0, x = 0, Ql = [{ line: 1, column: 1 }], cl = 0, bd = [], o = 0;
        if ("startRule" in Y) {
          if (!(Y.startRule in n))
            throw new Error(`Can't start parsing from rule "` + Y.startRule + '".');
          V = n[Y.startRule];
        }
        function r(m, W) {
          return { type: "literal", text: m, ignoreCase: W };
        }
        function Xd(m, W, i) {
          return { type: "class", parts: m, inverted: W, ignoreCase: i };
        }
        function Qd(m) {
          return { type: "other", description: m };
        }
        function Pd(m) {
          var W, i = Ql[m];
          if (i)
            return i;
          for (W = m - 1; !Ql[W]; )
            W--;
          for (i = { line: (i = Ql[W]).line, column: i.column }; W < m; )
            d.charCodeAt(W) === 10 ? (i.line++, i.column = 1) : i.column++, W++;
          return Ql[m] = i, i;
        }
        function gd(m, W) {
          var i = Pd(m), R = Pd(W);
          return { start: { offset: m, line: i.line, column: i.column }, end: { offset: W, line: R.line, column: R.column } };
        }
        function h(m) {
          b < cl || (b > cl && (cl = b, bd = []), bd.push(m));
        }
        function jd() {
          var m, W, i, R, f, p;
          if (m = b, (W = md()) !== l) {
            for (i = [], d.length > b ? (R = d.charAt(b), b++) : (R = l, o === 0 && h(a)); R !== l; )
              i.push(R), d.length > b ? (R = d.charAt(b), b++) : (R = l, o === 0 && h(a));
            i !== l ? (x = m, p = i, (f = W).root = !0, p && (f.label = p.join("")), m = W = f) : (b = m, m = l);
          } else
            b = m, m = l;
          return m;
        }
        function Pl() {
          var m, W, i;
          return m = b, d.charCodeAt(b) === 62 ? (W = J, b++) : (W = l, o === 0 && h(y)), W === l && (d.charCodeAt(b) === 60 ? (W = C, b++) : (W = l, o === 0 && h(T)), W === l && (d.charCodeAt(b) === 61 ? (W = E, b++) : (W = l, o === 0 && h(ll)))), W !== l && (i = Zl()) !== l ? (x = m, m = W = { type: "success", mod: W, expr: i }) : (b = m, m = l), m;
        }
        function gl() {
          var m, W, i, R;
          return m = b, d.charCodeAt(b) === 102 ? (W = "f", b++) : (W = l, o === 0 && h(ol)), W !== l ? (d.charCodeAt(b) === 62 ? (i = J, b++) : (i = l, o === 0 && h(y)), i === l && (d.charCodeAt(b) === 60 ? (i = C, b++) : (i = l, o === 0 && h(T)), i === l && (d.charCodeAt(b) === 61 ? (i = E, b++) : (i = l, o === 0 && h(ll)))), i === l && (i = null), i !== l && (R = Zl()) !== l ? (x = m, m = W = { type: "failure", mod: i, expr: R }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function Ed() {
          var m, W, i, R;
          return m = b, d.substr(b, 2) === "cs" ? (W = "cs", b += 2) : (W = l, o === 0 && h(xl)), W !== l ? (d.charCodeAt(b) === 62 ? (i = J, b++) : (i = l, o === 0 && h(y)), i === l && (d.charCodeAt(b) === 60 ? (i = C, b++) : (i = l, o === 0 && h(T)), i === l && (d.charCodeAt(b) === 61 ? (i = E, b++) : (i = l, o === 0 && h(ll)))), i === l && (i = null), i !== l && (R = Zl()) !== l ? (x = m, m = W = { type: "crit", mod: i, expr: R }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function Id() {
          var m, W, i, R;
          return m = b, d.substr(b, 2) === "cf" ? (W = "cf", b += 2) : (W = l, o === 0 && h(Kl)), W !== l ? (d.charCodeAt(b) === 62 ? (i = J, b++) : (i = l, o === 0 && h(y)), i === l && (d.charCodeAt(b) === 60 ? (i = C, b++) : (i = l, o === 0 && h(T)), i === l && (d.charCodeAt(b) === 61 ? (i = E, b++) : (i = l, o === 0 && h(ll)))), i === l && (i = null), i !== l && (R = Zl()) !== l ? (x = m, m = W = { type: "critfail", mod: i, expr: R }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function jl() {
          var m, W, i, R;
          return m = b, d.charCodeAt(b) === 107 ? (W = "k", b++) : (W = l, o === 0 && h(ul)), W !== l ? (d.charCodeAt(b) === 108 ? (i = "l", b++) : (i = l, o === 0 && h(zl)), i === l && (d.charCodeAt(b) === 104 ? (i = "h", b++) : (i = l, o === 0 && h(Xl))), i === l && (i = null), i !== l ? ((R = Zl()) === l && (R = null), R !== l ? (x = m, m = W = { type: "keep", highlow: i, expr: R || _d }) : (b = m, m = l)) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function El() {
          var m, W, i, R;
          return m = b, d.charCodeAt(b) === 100 ? (W = "d", b++) : (W = l, o === 0 && h(A)), W !== l ? (d.charCodeAt(b) === 108 ? (i = "l", b++) : (i = l, o === 0 && h(zl)), i === l && (d.charCodeAt(b) === 104 ? (i = "h", b++) : (i = l, o === 0 && h(Xl))), i === l && (i = null), i !== l ? ((R = Zl()) === l && (R = null), R !== l ? (x = m, m = W = { type: "drop", highlow: i, expr: R || _d }) : (b = m, m = l)) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function cd() {
          var m, W, i, R, f, p, v, H, S, B;
          if (m = b, (W = Zd()) !== l) {
            for (i = [], R = b, (f = z()) !== l ? (d.charCodeAt(b) === 43 ? (p = Bl, b++) : (p = l, o === 0 && h(Ul)), p !== l && (v = z()) !== l && (H = Zd()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l); R !== l; )
              i.push(R), R = b, (f = z()) !== l ? (d.charCodeAt(b) === 43 ? (p = Bl, b++) : (p = l, o === 0 && h(Ul)), p !== l && (v = z()) !== l && (H = Zd()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l);
            i !== l ? (x = m, S = W, m = W = (B = i).length == 0 ? S : { head: S, type: "diceExpression", ops: B.map((w) => ({ type: "math", op: w[1], tail: w[3] })) }) : (b = m, m = l);
          } else
            b = m, m = l;
          return m;
        }
        function Zd() {
          var m;
          return (m = Td()) === l && (m = md()), m;
        }
        function Td() {
          var m, W, i;
          return m = b, W = function() {
            var R, f, p, v, H;
            if (R = b, f = function() {
              var S, B, w, P, U, I;
              if (S = b, B = function() {
                var k, O, g, q;
                return k = b, (O = Zl()) === l && (O = null), O !== l ? (d.charCodeAt(b) === 100 ? (g = "d", b++) : (g = l, o === 0 && h(A)), g !== l ? (q = function() {
                  var dl, M;
                  return dl = b, d.charCodeAt(b) === 70 ? (M = "F", b++) : (M = l, o === 0 && h(rb)), M === l && (d.charCodeAt(b) === 102 ? (M = "f", b++) : (M = l, o === 0 && h(ol))), M !== l && (x = dl, M = { type: "fate" }), M;
                }(), q === l && (q = function() {
                  var dl, M;
                  return dl = b, d.charCodeAt(b) === 37 ? (M = "%", b++) : (M = l, o === 0 && h(_l)), M !== l && (x = dl, M = { type: "number", value: "100" }), M;
                }(), q === l && (q = Zl())), q !== l ? (x = k, k = O = { die: q, count: O || { type: "number", value: 1 }, type: "die" }) : (b = k, k = l)) : (b = k, k = l)) : (b = k, k = l), k;
              }(), B !== l) {
                for (w = [], (P = Dd()) === l && (P = Ad()) === l && (P = Md()) === l && (P = qd()) === l && (P = Od()); P !== l; )
                  w.push(P), (P = Dd()) === l && (P = Ad()) === l && (P = Md()) === l && (P = qd()) === l && (P = Od());
                w !== l ? (x = S, I = w, (U = B).mods = (U.mods || []).concat(I), S = B = U) : (b = S, S = l);
              } else
                b = S, S = l;
              return S;
            }(), f !== l) {
              for (p = [], (v = El()) === l && (v = jl()) === l && (v = Pl()) === l && (v = gl()) === l && (v = Id()) === l && (v = Ed()); v !== l; )
                p.push(v), (v = El()) === l && (v = jl()) === l && (v = Pl()) === l && (v = gl()) === l && (v = Id()) === l && (v = Ed());
              p !== l ? ((v = function() {
                var S, B, w, P, U;
                return S = b, d.charCodeAt(b) === 109 ? (B = "m", b++) : (B = l, o === 0 && h(Cl)), B !== l ? (d.charCodeAt(b) === 116 ? (w = "t", b++) : (w = l, o === 0 && h(wl)), w === l && (w = null), w !== l ? ((P = ed()) === l && (P = null), P !== l ? (U = function() {
                  var I, k, O;
                  return I = b, d.charCodeAt(b) === 62 ? (k = J, b++) : (k = l, o === 0 && h(y)), k === l && (d.charCodeAt(b) === 60 ? (k = C, b++) : (k = l, o === 0 && h(T)), k === l && (d.charCodeAt(b) === 61 ? (k = E, b++) : (k = l, o === 0 && h(ll)))), k !== l && (O = Zl()) !== l ? (x = I, I = k = { mod: k, expr: O }) : (b = I, I = l), I;
                }(), U === l && (U = null), U !== l ? (x = S, S = B = function(I, k, O) {
                  const g = { type: "match", min: k || { type: "number", value: 2 }, count: !!I };
                  return O && (g.mod = O.mod, g.expr = O.expr), g;
                }(w, P, U)) : (b = S, S = l)) : (b = S, S = l)) : (b = S, S = l)) : (b = S, S = l), S;
              }()) === l && (v = null), v !== l ? (H = function() {
                var S, B, w;
                return S = b, d.charCodeAt(b) === 115 ? (B = "s", b++) : (B = l, o === 0 && h(yb)), B !== l ? (d.charCodeAt(b) === 97 ? (w = "a", b++) : (w = l, o === 0 && h(fb)), w === l && (d.charCodeAt(b) === 100 ? (w = "d", b++) : (w = l, o === 0 && h(A))), w === l && (w = null), w !== l ? (x = S, S = B = w == "d" ? { type: "sort", asc: !1 } : { type: "sort", asc: !0 }) : (b = S, S = l)) : (b = S, S = l), S;
              }(), H === l && (H = null), H !== l ? (x = R, R = f = function(S, B, w, P) {
                const U = B.filter((I) => ["success", "failure"].includes(I.type));
                return B = B.filter((I) => !U.includes(I)), S.mods = (S.mods || []).concat(B), U.length > 0 && (S.targets = U), w && (S.match = w), P && (S.sort = P), S;
              }(f, p, v, H)) : (b = R, R = l)) : (b = R, R = l)) : (b = R, R = l);
            } else
              b = R, R = l;
            return R;
          }(), W !== l && z() !== l ? ((i = Il()) === l && (i = null), i !== l ? (x = m, m = W = N(W, i)) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function Md() {
          var m, W, i;
          return m = b, d.charCodeAt(b) === 33 ? (W = "!", b++) : (W = l, o === 0 && h(vb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (x = m, m = W = { type: "explode", target: i }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function Dd() {
          var m, W, i;
          return m = b, d.substr(b, 2) === "!!" ? (W = "!!", b += 2) : (W = l, o === 0 && h(Lb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (x = m, m = W = { type: "compound", target: i }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function Ad() {
          var m, W, i;
          return m = b, d.substr(b, 2) === "!p" ? (W = "!p", b += 2) : (W = l, o === 0 && h(Nb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (x = m, m = W = { type: "penetrate", target: i }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function Od() {
          var m, W, i;
          return m = b, d.charCodeAt(b) === 114 ? (W = "r", b++) : (W = l, o === 0 && h(Sb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (x = m, m = W = { type: "reroll", target: i || $d }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function qd() {
          var m, W, i;
          return m = b, d.substr(b, 2) === "ro" ? (W = "ro", b += 2) : (W = l, o === 0 && h(Fb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (x = m, m = W = { type: "rerollOnce", target: i || $d }) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function hl() {
          var m, W, i;
          return m = b, d.charCodeAt(b) === 62 ? (W = J, b++) : (W = l, o === 0 && h(y)), W === l && (d.charCodeAt(b) === 60 ? (W = C, b++) : (W = l, o === 0 && h(T)), W === l && (d.charCodeAt(b) === 61 ? (W = E, b++) : (W = l, o === 0 && h(ll)))), W === l && (W = null), W !== l && (i = Zl()) !== l ? (x = m, m = W = { type: "target", mod: W, value: i }) : (b = m, m = l), m;
        }
        function Zl() {
          var m;
          return (m = ud()) === l && (m = ed()), m;
        }
        function md() {
          var m;
          return (m = function() {
            var W, i, R, f;
            return W = b, d.substr(b, 2) === "[[" ? (i = "[[", b += 2) : (i = l, o === 0 && h(t)), i !== l && (R = md()) !== l ? (d.substr(b, 2) === "]]" ? (f = "]]", b += 2) : (f = l, o === 0 && h(L)), f !== l ? (x = W, W = i = { type: "inline", expr: R }) : (b = W, W = l)) : (b = W, W = l), W;
          }()) === l && (m = Gd()) === l && (m = ud()), m;
        }
        function ud() {
          var m, W, i, R, f, p, v;
          return m = b, d.charCodeAt(b) === 40 ? (W = "(", b++) : (W = l, o === 0 && h(Ld)), W !== l && (i = Gd()) !== l ? (d.charCodeAt(b) === 41 ? (R = ")", b++) : (R = l, o === 0 && h(Nd)), R !== l && z() !== l ? ((f = Il()) === l && (f = null), f !== l ? (x = m, p = i, (v = f) && (p.label = v), m = W = p) : (b = m, m = l)) : (b = m, m = l)) : (b = m, m = l), m;
        }
        function Gd() {
          var m, W, i, R, f, p, v, H;
          if (m = b, (W = Wd()) !== l) {
            for (i = [], R = b, (f = z()) !== l ? (d.charCodeAt(b) === 43 ? (p = Bl, b++) : (p = l, o === 0 && h(Ul)), p === l && (d.charCodeAt(b) === 45 ? (p = "-", b++) : (p = l, o === 0 && h(ld))), p !== l && (v = z()) !== l && (H = Wd()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l); R !== l; )
              i.push(R), R = b, (f = z()) !== l ? (d.charCodeAt(b) === 43 ? (p = Bl, b++) : (p = l, o === 0 && h(Ul)), p === l && (d.charCodeAt(b) === 45 ? (p = "-", b++) : (p = l, o === 0 && h(ld))), p !== l && (v = z()) !== l && (H = Wd()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l);
            i !== l ? (x = m, m = W = dd(W, i)) : (b = m, m = l);
          } else
            b = m, m = l;
          return m;
        }
        function Wd() {
          var m, W, i, R, f, p, v, H;
          if (m = b, (W = id()) !== l) {
            for (i = [], R = b, (f = z()) !== l ? (d.charCodeAt(b) === 42 ? (p = "*", b++) : (p = l, o === 0 && h(Sd)), p === l && (d.charCodeAt(b) === 47 ? (p = "/", b++) : (p = l, o === 0 && h(Fd))), p !== l && (v = z()) !== l && (H = id()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l); R !== l; )
              i.push(R), R = b, (f = z()) !== l ? (d.charCodeAt(b) === 42 ? (p = "*", b++) : (p = l, o === 0 && h(Sd)), p === l && (d.charCodeAt(b) === 47 ? (p = "/", b++) : (p = l, o === 0 && h(Fd))), p !== l && (v = z()) !== l && (H = id()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l);
            i !== l ? (x = m, m = W = dd(W, i)) : (b = m, m = l);
          } else
            b = m, m = l;
          return m;
        }
        function id() {
          var m, W, i, R, f, p, v, H;
          if (m = b, (W = nd()) !== l) {
            for (i = [], R = b, (f = z()) !== l ? (d.substr(b, 2) === kl ? (p = kl, b += 2) : (p = l, o === 0 && h(rd)), p === l && (d.charCodeAt(b) === 37 ? (p = "%", b++) : (p = l, o === 0 && h(_l))), p !== l && (v = z()) !== l && (H = nd()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l); R !== l; )
              i.push(R), R = b, (f = z()) !== l ? (d.substr(b, 2) === kl ? (p = kl, b += 2) : (p = l, o === 0 && h(rd)), p === l && (d.charCodeAt(b) === 37 ? (p = "%", b++) : (p = l, o === 0 && h(_l))), p !== l && (v = z()) !== l && (H = nd()) !== l ? R = f = [f, p, v, H] : (b = R, R = l)) : (b = R, R = l);
            i !== l ? (x = m, m = W = dd(W, i)) : (b = m, m = l);
          } else
            b = m, m = l;
          return m;
        }
        function nd() {
          var m;
          return (m = function() {
            var W, i, R, f, p;
            return W = b, i = function() {
              var v;
              return d.substr(b, 5) === Hd ? (v = Hd, b += 5) : (v = l, o === 0 && h(Hb)), v === l && (d.substr(b, 4) === xd ? (v = xd, b += 4) : (v = l, o === 0 && h(xb)), v === l && (d.substr(b, 5) === Kd ? (v = Kd, b += 5) : (v = l, o === 0 && h(Kb)), v === l && (d.substr(b, 3) === "abs" ? (v = "abs", b += 3) : (v = l, o === 0 && h(Cb))))), v;
            }(), i !== l && z() !== l ? (d.charCodeAt(b) === 40 ? (R = "(", b++) : (R = l, o === 0 && h(Ld)), R !== l && z() !== l && (f = Gd()) !== l && z() !== l ? (d.charCodeAt(b) === 41 ? (p = ")", b++) : (p = l, o === 0 && h(Nd)), p !== l ? (x = W, W = i = { type: "mathfunction", op: i, expr: f }) : (b = W, W = l)) : (b = W, W = l)) : (b = W, W = l), W;
          }()) === l && (m = function() {
            var W, i, R;
            return W = b, i = function() {
              var f, p, v, H, S, B, w, P;
              if (f = b, p = function() {
                var U, I, k, O, g, q, dl, M, Tl;
                if (U = b, d.charCodeAt(b) === 123 ? (I = "{", b++) : (I = l, o === 0 && h(bl)), I !== l)
                  if (z() !== l)
                    if ((k = cd()) !== l) {
                      for (O = [], g = b, (q = z()) !== l ? (d.charCodeAt(b) === 44 ? (dl = ",", b++) : (dl = l, o === 0 && h(Gl)), dl !== l && (M = z()) !== l && (Tl = cd()) !== l ? g = q = [q, dl, M, Tl] : (b = g, g = l)) : (b = g, g = l); g !== l; )
                        O.push(g), g = b, (q = z()) !== l ? (d.charCodeAt(b) === 44 ? (dl = ",", b++) : (dl = l, o === 0 && h(Gl)), dl !== l && (M = z()) !== l && (Tl = cd()) !== l ? g = q = [q, dl, M, Tl] : (b = g, g = l)) : (b = g, g = l);
                      O !== l && (g = z()) !== l ? (d.charCodeAt(b) === 125 ? (q = "}", b++) : (q = l, o === 0 && h(Jb)), q !== l ? (x = U, U = I = { rolls: [k, ...O.map((kb) => kb[3])], type: "group" }) : (b = U, U = l)) : (b = U, U = l);
                    } else
                      b = U, U = l;
                  else
                    b = U, U = l;
                else
                  b = U, U = l;
                return U;
              }(), p !== l) {
                for (v = [], (H = jl()) === l && (H = El()) === l && (H = Pl()) === l && (H = gl()); H !== l; )
                  v.push(H), (H = jl()) === l && (H = El()) === l && (H = Pl()) === l && (H = gl());
                v !== l && (H = z()) !== l ? ((S = Il()) === l && (S = null), S !== l ? (x = f, B = p, P = S, (w = v).length > 0 && (B.mods = (B.mods || []).concat(w)), P && (B.label = P), f = p = B) : (b = f, f = l)) : (b = f, f = l);
              } else
                b = f, f = l;
              return f;
            }(), i === l && (i = Td()) === l && (i = ed()), i !== l && z() !== l ? ((R = Il()) === l && (R = null), R !== l ? (x = W, W = i = N(i, R)) : (b = W, W = l)) : (b = W, W = l), W;
          }()) === l && (m = ud()), m;
        }
        function ed() {
          var m, W, i, R;
          if (o++, m = b, d.charCodeAt(b) === 45 ? (W = "-", b++) : (W = l, o === 0 && h(ld)), W === l && (W = null), W !== l) {
            if (i = [], Cd.test(d.charAt(b)) ? (R = d.charAt(b), b++) : (R = l, o === 0 && h(wd)), R !== l)
              for (; R !== l; )
                i.push(R), Cd.test(d.charAt(b)) ? (R = d.charAt(b), b++) : (R = l, o === 0 && h(wd));
            else
              i = l;
            i !== l ? (x = m, m = W = { type: "number", value: parseInt(d.substring(x, b), 10) }) : (b = m, m = l);
          } else
            b = m, m = l;
          return o--, m === l && (W = l, o === 0 && h(wb)), m;
        }
        function Il() {
          var m, W, i, R;
          if (m = b, d.charCodeAt(b) === 91 ? (W = "[", b++) : (W = l, o === 0 && h(zb)), W !== l) {
            if (i = [], zd.test(d.charAt(b)) ? (R = d.charAt(b), b++) : (R = l, o === 0 && h(Bd)), R !== l)
              for (; R !== l; )
                i.push(R), zd.test(d.charAt(b)) ? (R = d.charAt(b), b++) : (R = l, o === 0 && h(Bd));
            else
              i = l;
            i !== l ? (d.charCodeAt(b) === 93 ? (R = "]", b++) : (R = l, o === 0 && h(Bb)), R !== l ? (x = m, m = W = i.join("")) : (b = m, m = l)) : (b = m, m = l);
          } else
            b = m, m = l;
          return m;
        }
        function z() {
          var m, W;
          for (o++, m = [], Ud.test(d.charAt(b)) ? (W = d.charAt(b), b++) : (W = l, o === 0 && h(kd)); W !== l; )
            m.push(W), Ud.test(d.charAt(b)) ? (W = d.charAt(b), b++) : (W = l, o === 0 && h(kd));
          return o--, m === l && (W = l, o === 0 && h(Ub)), m;
        }
        const $d = { type: "target", mod: "=", value: { type: "number", value: 1 } }, _d = { type: "number", value: 1 };
        if ((c = V()) !== l && b === d.length)
          return c;
        throw c !== l && b < d.length && h({ type: "end" }), ad = bd, sd = cl < d.length ? d.charAt(cl) : null, lb = cl < d.length ? gd(cl, cl + 1) : gd(cl, cl), new e(e.buildMessage(ad, sd), ad, sd, lb);
        var ad, sd, lb;
      } };
    } }, G = {};
    return function s(e) {
      var d = G[e];
      if (d !== void 0)
        return d.exports;
      var Y = G[e] = { exports: {} };
      return u[e].call(Y.exports, Y, Y.exports, s), Y.exports;
    }(607);
  })());
})(ib);
var nb = ib.exports;
const eb = /* @__PURE__ */ Ob(nb), qb = /* @__PURE__ */ gb({
  __proto__: null,
  default: eb
}, [nb]), $b = eb || qb, { DiceRoller: ab } = $b, _b = {
  maxNotationLength: 200,
  maxDiceCount: 100,
  maxSides: 100,
  maxRerollDepth: 100
}, lX = /* @__PURE__ */ new Set(["explode", "compound", "penetrate", "reroll", "rerollOnce"]);
class dX extends Error {
  constructor(X, u = 400) {
    super(X), this.name = "RollNotationError", this.statusCode = u;
  }
}
class Rd extends Error {
  constructor(X) {
    super("Parser requested another physical die."), this.name = "PhysicalRollRequest", this.requestedRoll = X;
  }
}
const K = (Z) => new dX(Z), $l = (Z) => ({
  ..._b,
  ...Z
}), Xb = (Z) => {
  if (Z !== void 0)
    return JSON.parse(JSON.stringify(Z));
}, bX = (Z) => Z.replace(/d00(?!\d)/gi, "d100").replace(/dFATE\b/gi, "dF"), sb = (Z) => {
  if (typeof Z != "string")
    throw K("Roll notation must be a string.");
  const X = Z.trim();
  if (!X)
    throw K("Roll notation cannot be empty.");
  return X;
}, Vb = (Z) => {
  const X = Z.match(/@[A-Za-z_][\w.]*/u) || Z.match(/@/u);
  if (X)
    throw K(`Unsupported roll notation contains unresolved app token '${X[0]}'. Resolve app tokens before rolling.`);
}, Rb = (Z, X) => {
  if (Z.length > X.maxNotationLength)
    throw K(`Roll notation is too long. Maximum length is ${X.maxNotationLength} characters.`);
}, Vl = (Z) => Z !== null && typeof Z == "object", td = (Z) => Vl(Z) ? typeof Z.label == "string" && Z.label.trim() ? !0 : Object.values(Z).some((X) => Array.isArray(X) ? X.some(td) : td(X)) : !1, od = (Z) => Vl(Z) ? lX.has(Z.type) ? !0 : Object.values(Z).some((X) => Array.isArray(X) ? X.some(od) : od(X)) : !1, il = (Z) => Vl(Z) ? Z.type === "die" ? !0 : Object.values(Z).some((X) => Array.isArray(X) ? X.some(il) : il(X)) : !1, Jl = (Z, X) => {
  if (!Vl(Z))
    throw K(`Unsupported ${X}.`);
  switch (Z.type) {
    case "number":
      return Number(Z.value);
    case "expression": {
      let u = Jl(Z.head, X);
      for (const G of Z.ops || []) {
        const s = Jl(G.tail, X);
        switch (G.op) {
          case "+":
            u += s;
            break;
          case "-":
            u -= s;
            break;
          case "*":
            u *= s;
            break;
          case "/":
            u /= s;
            break;
          case "%":
            u %= s;
            break;
          case "**":
            u **= s;
            break;
          default:
            throw K(`Unsupported ${X} math operator '${G.op}'.`);
        }
      }
      return u;
    }
    case "mathfunction": {
      const u = Jl(Z.expr, X);
      switch (Z.op) {
        case "floor":
          return Math.floor(u);
        case "ceil":
          return Math.ceil(u);
        case "round":
          return Math.round(u);
        case "abs":
          return Math.abs(u);
        default:
          throw K(`Unsupported ${X} math function '${Z.op}'.`);
      }
    }
    default:
      throw K(`Unsupported ${X}. Dice count and sides must be static numbers.`);
  }
}, vd = (Z, X) => {
  if (!Number.isInteger(Z) || Z < 1)
    throw K(`Invalid ${X}. Expected a positive integer.`);
  return Z;
}, XX = (Z, X) => {
  if ((Z == null ? void 0 : Z.type) === "fate")
    return "fate";
  const u = vd(Jl(Z, "die sides"), "die sides");
  if (u < 2)
    throw K("Invalid die sides. Dice must have at least 2 sides.");
  if (u > X.maxSides)
    throw K(`Roll notation exceeds the maximum die size of d${X.maxSides}.`);
  return u;
}, cX = (Z, X) => `${Z}d${X === "fate" ? "F" : X}`, ZX = (Z, X, u) => {
  const G = vd(Jl(Z.count, "dice count"), "dice count"), s = XX(Z.die, X);
  return {
    qty: G,
    sides: s === "fate" ? "fate" : `d${s}`,
    modifier: 0,
    notation: cX(G, s),
    data: void 0,
    parserPath: u
  };
}, Rl = (Z, X, u, G = "root") => {
  var s, e;
  if (Vl(Z))
    switch (Z.type) {
      case "die":
        X.push(ZX(Z, u, G));
        for (const d of [...Z.mods || [], ...Z.targets || []]) {
          if (d.expr && il(d.expr))
            throw K("Unsupported roll notation. Modifier expressions cannot require additional physical dice.");
          if ((s = d.target) != null && s.value && il(d.target.value))
            throw K("Unsupported roll notation. Reroll target expressions cannot require additional physical dice.");
        }
        if ((e = Z.match) != null && e.expr && il(Z.match.expr))
          throw K("Unsupported roll notation. Match expressions cannot require additional physical dice.");
        break;
      case "diceExpression":
      case "expression":
        Rl(Z.head, X, u, `${G}.head`);
        for (let d = 0; d < (Z.ops || []).length; d++)
          Rl(Z.ops[d].tail, X, u, `${G}.ops.${d}`);
        break;
      case "group":
        for (let d = 0; d < (Z.rolls || []).length; d++)
          Rl(Z.rolls[d], X, u, `${G}.rolls.${d}`);
        for (const d of Z.mods || [])
          if (d.expr && il(d.expr))
            throw K("Unsupported roll notation. Group modifier expressions cannot require additional physical dice.");
        break;
      case "mathfunction":
        Rl(Z.expr, X, u, `${G}.expr`);
        break;
      case "inline":
        Rl(Z.expr, X, u, `${G}.expr`);
        break;
      case "number":
        break;
      default:
        throw K(`Unsupported roll notation node '${Z.type}'.`);
    }
}, mX = (Z) => Z.reduce((X, u) => X + u.qty, 0), uX = (Z) => {
  var u;
  if ((Z == null ? void 0 : Z.type) !== "expression" || il({ ...Z, head: void 0 }) || !il(Z.head))
    return 0;
  let X = 0;
  for (const G of Z.ops || []) {
    if (((u = G.tail) == null ? void 0 : u.type) !== "number" || !["+", "-"].includes(G.op))
      return 0;
    X += G.op === "+" ? Number(G.tail.value) : -Number(G.tail.value);
  }
  return X;
}, GX = (Z, X, u) => {
  if (Z.length !== 1)
    return Z;
  const G = uX(X);
  return G ? [{
    ...Z[0],
    modifier: G,
    notation: u.replace(/\s+/g, "")
  }] : Z;
}, WX = (Z, X) => {
  let u = (X.match(/[dD](?:00|%)(?!\d)/g) || []).length;
  return u ? Z.map((G) => u > 0 && G.sides === "d100" ? (u--, { ...G, data: "single" }) : G) : Z;
}, Yb = (Z, X) => {
  const u = mX(Z);
  if (u < 1)
    throw K("Roll notation must include at least one physical die.");
  if (u > X.maxDiceCount)
    throw K(`Roll notation requests ${u} dice. Maximum dice count is ${X.maxDiceCount}.`);
  return Z.forEach((G) => {
    vd(Number(G.qty), "dice count");
    const s = G.sides;
    if (s === "fate")
      return;
    const e = String(s).replace(/\D/g, "");
    if (!Number.isInteger(s) && !e && typeof s == "string")
      return;
    const d = Number.isInteger(s) ? s : parseInt(e, 10);
    if (!Number.isInteger(d) || d < 2)
      throw K("Invalid die sides. Dice must have at least 2 sides.");
    if (d > X.maxSides)
      throw K(`Roll notation exceeds the maximum die size of d${X.maxSides}.`);
  }), Z;
}, iX = (Z, X = {}) => {
  const u = $l(X), G = sb(Z);
  Rb(G, u), Vb(G);
  const s = bX(G), e = new ab(null, u.maxRerollDepth);
  let d;
  try {
    d = e.parse(s);
  } catch (l) {
    throw K(`Invalid roll notation '${G}': ${l.message}`);
  }
  if (td(d))
    throw K(`Unsupported roll notation '${G}': unexpected trailing text '${d.label.trim()}'.`);
  if (od(d) && !u.allowRerollingMods)
    throw K(`Reroll/explode notation requires a roll path that can append extra physical dice. Use /api/roll or enable allowRerollingMods. Maximum configured reroll/explode depth is ${u.maxRerollDepth}.`);
  const Y = [];
  Rl(d, Y, u), Yb(Y, u);
  const c = GX(
    WX(Y, G),
    d,
    G
  );
  return {
    originalNotation: G,
    parserNotation: s,
    diceGroups: c,
    parsedTree: d,
    mode: "advanced"
  };
}, cb = (Z, X, u) => {
  const G = Z === "" || Z === void 0 ? X : Number(Z);
  if (Number.isNaN(G) || !Number.isInteger(G) || G < 1)
    throw K(`Invalid notation: ${u}`);
  return G;
}, Zb = (Z, X = [], u = {}) => {
  const G = $l(u), s = typeof Z == "string" ? sb(Z) : null;
  s && (Rb(s, G), Vb(s));
  const e = Array.isArray(Z) ? Z : s.split(",").map((V) => V.trim()).filter(Boolean), d = [], Y = /^(\d*)[dD](00|%)([+-]\d+)?$/i, c = /^(\d*)[dD](f+[ate]*)([+-]\d+)?$/i, l = /^(\d*)[dD](\d+)([+-]\d+)?$/i, n = /^(\d*)[dD]([\d\w]+)([+-]\d+)?$/i;
  return e.forEach((V) => {
    if (typeof V == "object" && V !== null) {
      if (!V.sides)
        throw K("Roll notation is missing sides");
      d.push({ qty: 1, modifier: 0, ...V });
      return;
    }
    const a = String(V).trim().replace(/\s+/g, ""), t = a.match(Y) || a.match(c) || a.match(l) || a.match(n);
    if (!t || t.length < 3)
      throw K(`Invalid notation: ${V}`);
    let L = 0;
    t[3] && (L = Number(t[3]));
    const N = {
      qty: cb(t[1], 1, V),
      modifier: L,
      notation: a
    };
    if (a.match(Y))
      N.sides = "d100", N.data = "single";
    else if (a.match(c))
      N.sides = "fate";
    else if (a.match(l)) {
      const J = cb(t[2], 1, V);
      N.sides = `d${J}`;
    } else if (X.includes(t[2]))
      N.sides = t[2];
    else
      throw K(`Invalid notation: ${V}`);
    d.push(N);
  }), Yb(d, G), d;
}, Ml = (Z, X = [], u = {}) => {
  if (typeof Z == "string")
    try {
      return iX(Z, u);
    } catch (G) {
      try {
        const s = Zb(Z, X, u);
        return {
          originalNotation: Z.trim(),
          parserNotation: null,
          diceGroups: s,
          parsedTree: null,
          mode: "legacy",
          advancedError: G.message
        };
      } catch {
        throw G;
      }
    }
  return {
    originalNotation: Array.isArray(Z) ? Z.join(",") : "",
    parserNotation: null,
    diceGroups: Zb(Z, X, u),
    parsedTree: null,
    mode: "legacy"
  };
}, mb = (Z) => {
  if (Z.sides === "fate")
    return "fate";
  if (Number.isInteger(Z.sides))
    return Z.sides;
  if (typeof Z.sides == "string") {
    const X = parseInt(Z.sides.replace(/\D/g, ""), 10);
    if (Number.isInteger(X))
      return X;
  }
  if (typeof Z.dieType == "string") {
    const X = parseInt(Z.dieType.replace(/\D/g, ""), 10);
    if (Number.isInteger(X))
      return X;
  }
  throw K("Unable to determine die sides from physical roll results.");
}, tb = (Z) => Array.isArray(Z) ? Z.flatMap((X) => Array.isArray(X.rolls) ? X.rolls : (X == null ? void 0 : X.value) !== void 0 ? [X] : []) : [], nX = ({ sides: Z, order: X, physicalRollIndex: u, value: G }) => ({
  critical: G === Z ? "success" : G === 1 ? "failure" : null,
  die: Z,
  matched: !1,
  order: X,
  physicalRollIndex: u,
  roll: G,
  success: null,
  successes: 0,
  failures: 0,
  type: "roll",
  valid: !0,
  value: G
}), eX = ({ order: Z, physicalRollIndex: X, value: u }) => ({
  matched: !1,
  order: Z,
  physicalRollIndex: X,
  roll: u,
  success: null,
  successes: 0,
  failures: 0,
  type: "fateroll",
  valid: !0,
  value: u
}), hd = (Z, X = []) => (Vl(Z) && ((Z.type === "roll" || Z.type === "fateroll") && Number.isInteger(Z.physicalRollIndex) && X.push(Z), Object.values(Z).forEach((u) => {
  Array.isArray(u) ? u.forEach((G) => hd(G, X)) : Vl(u) && hd(u, X);
})), X), aX = (Z) => {
  const {
    critical: X,
    drop: u,
    explode: G,
    failures: s,
    matched: e,
    order: d,
    physicalRollIndex: Y,
    reroll: c,
    success: l,
    successes: n,
    valid: V,
    value: a
  } = Z;
  return {
    critical: X ?? null,
    drop: !!u,
    explode: !!G,
    failures: s || 0,
    matched: !!e,
    order: d,
    physicalRollIndex: Y,
    reroll: !!c,
    success: l ?? null,
    successes: n || 0,
    valid: V !== !1,
    value: a
  };
}, sX = (Z, X) => ({
  critical: null,
  consumed: !0,
  drop: !1,
  explode: !1,
  failures: 0,
  folded: !0,
  matched: !1,
  order: null,
  physicalRollIndex: X,
  reroll: !1,
  success: null,
  successes: 0,
  valid: !1,
  value: Number(Z.value)
}), VX = (Z, X, u) => {
  const G = tb(Z), s = hd(X), e = /* @__PURE__ */ new Set();
  s.forEach((d) => {
    const Y = G[d.physicalRollIndex];
    Y && (e.add(d.physicalRollIndex), Object.assign(Y, {
      parser: aX(d)
    }));
  });
  for (let d = 0; d < u; d++) {
    if (e.has(d))
      continue;
    const Y = G[d];
    Y && (Y.parser = sX(Y, d));
  }
  return s;
}, RX = (Z, X, u = {}) => {
  const G = $l(u), s = tb(X);
  let e = 0;
  const d = new ab(null, G.maxRerollDepth);
  d.generateDiceRoll = (Y, c) => {
    const l = s[e];
    if (!l)
      throw new Rd({ sides: Y, qty: 1 });
    const n = mb(l);
    if (n !== Y)
      throw K(`Physical roll d${n} did not match parser-requested d${Y}.`);
    const V = Number(l.value);
    if (!Number.isInteger(V) || V < 1 || V > Y)
      throw K(`Invalid d${Y} physical result '${l.value}'.`);
    return nX({
      sides: Y,
      order: c,
      physicalRollIndex: e++,
      value: V
    });
  }, d.generateFateRoll = (Y) => {
    const c = s[e];
    if (!c)
      throw new Rd({ sides: "fate", qty: 1 });
    const l = mb(c);
    if (l !== "fate")
      throw K(`Physical roll d${l} did not match parser-requested fate die.`);
    const n = Number(c.value);
    if (![-1, 0, 1].includes(n))
      throw K(`Invalid fate die result '${c.value}'.`);
    return eX({
      order: Y,
      physicalRollIndex: e++,
      value: n
    });
  };
  try {
    const Y = d.rollParsed(Z);
    if (!u.allowUnusedPhysicalRolls && e !== s.length)
      throw K("Physical roll results did not match parsed dice notation.");
    const c = VX(X, Y, e);
    return {
      complete: !0,
      consumed: e,
      result: Y,
      parserRolls: c
    };
  } catch (Y) {
    if (Y instanceof Rd)
      return {
        complete: !1,
        consumed: e,
        requestedRoll: Y.requestedRoll
      };
    throw Y;
  }
}, YX = (Z, X, u = {}) => {
  const G = $l(u), s = u.originalNotation || u.notation || "";
  if (!Z)
    return {
      value: X.reduce((c, l) => c + (Number(l.value) || 0), 0),
      notation: s,
      type: "legacy",
      success: null,
      successes: 0,
      failures: 0
    };
  const e = RX(Z, X, G);
  if (!e.complete) {
    const Y = e.requestedRoll.sides === "fate" ? "fate die" : `d${e.requestedRoll.sides}`;
    throw K(`Parser requested another physical ${Y}.`);
  }
  const d = e.result;
  return {
    value: d.value,
    notation: s,
    type: d.type,
    success: d.success ?? null,
    successes: d.successes || 0,
    failures: d.failures || 0,
    rolls: Xb(e.parserRolls),
    details: Xb(d)
  };
}, tX = {
  id: `dice-canvas-${Date.now()}`,
  // set the canvas id
  container: null,
  enableShadows: !0,
  // do dice cast shadows onto DiceBox mesh?
  shadowTransparency: 0.8,
  lightIntensity: 1,
  delay: 10,
  // delay between dice being generated - 0 causes stuttering and physics popping
  scale: 5,
  // scale the dice
  theme: "default",
  // can be a hex color or a pre-defined theme such as 'purpleRock'
  preloadThemes: [],
  externalThemes: {},
  // point to CDN paths
  themeColor: "#2e8555",
  // used for color values or named theme variants - not fully implemented yet // green: #2e8555 // yellow: #feea03
  offscreen: !0,
  // use offscreen canvas browser feature for performance improvements - will fallback to false based on feature detection
  assetPath: "/assets/dice-box/",
  // path to 'ammo', 'themes' folders and web workers
  // origin: location.origin,
  origin: typeof window < "u" ? window.location.origin : "",
  suspendSimulation: !1,
  maxNotationLength: 200,
  maxDiceCount: 100,
  maxSides: 100,
  maxRerollDepth: 100
}, oX = (Z) => {
  if (Z instanceof Float32Array)
    return Z;
  if (Array.isArray(Z))
    return new Float32Array(Z);
  if (Z != null && Z.data && Array.isArray(Z.data))
    return new Float32Array(Z.data);
  if ((Z == null ? void 0 : Z.type) === "Float32Array" && (Z == null ? void 0 : Z.encoding) === "base64") {
    let X;
    if (typeof atob == "function")
      X = atob(Z.data);
    else if (typeof Buffer < "u") {
      const G = Buffer.from(Z.data, "base64");
      return new Float32Array(G.buffer, G.byteOffset, G.byteLength / Float32Array.BYTES_PER_ELEMENT);
    }
    const u = new Uint8Array(X.length);
    for (let G = 0; G < X.length; G++)
      u[G] = X.charCodeAt(G);
    return new Float32Array(u.buffer);
  }
  throw new Error("Replay payload is missing Float32Array frame data.");
}, hX = (Z) => {
  const X = new Uint8Array(Z.buffer, Z.byteOffset, Z.byteLength);
  if (typeof Buffer < "u")
    return Buffer.from(X).toString("base64");
  let u = "";
  const G = 32768;
  for (let s = 0; s < X.length; s += G)
    u += String.fromCharCode(...X.subarray(s, s + G));
  return btoa(u);
}, pX = ({ notation: Z, results: X, finalResult: u, recording: G, config: s, canvas: e, theme: d, themeColor: Y }) => {
  var N;
  const c = G.frameData || new Float32Array(), l = G.frameRate || 60, n = ((N = G.renderDice) == null ? void 0 : N.length) || 0, V = 8, a = n ? c.length / (n * V) : 0, t = X.flatMap((J) => J.rolls || []), L = {
    value: X.reduce((J, y) => J + (Number(y.value) || 0), 0),
    notation: Array.isArray(Z) ? Z.join(",") : String(Z),
    type: "legacy",
    success: null,
    successes: 0,
    failures: 0
  };
  return {
    version: 1,
    metadata: {
      notation: Array.isArray(Z) ? Z.join(",") : String(Z),
      generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
      seed: null,
      theme: d,
      themeColor: Y,
      timedOut: !1,
      config: {
        width: (e == null ? void 0 : e.clientWidth) || (e == null ? void 0 : e.width),
        height: (e == null ? void 0 : e.clientHeight) || (e == null ? void 0 : e.height),
        scale: s.scale,
        frameRate: l,
        delay: s.delay,
        settleTimeout: s.settleTimeout
      },
      results: X,
      finalResult: u || L,
      rolls: t,
      renderDice: G.renderDice || [],
      frame: {
        type: "Float32Array",
        fields: ["id", "px", "py", "pz", "qx", "qy", "qz", "qw"],
        stride: V,
        dieCount: n,
        frameCount: a,
        frameRate: l,
        length: c.length,
        durationMs: Math.round(a * (1e3 / l))
      }
    },
    frames: {
      type: "Float32Array",
      encoding: "base64",
      littleEndian: !0,
      length: c.length,
      data: hX(c)
    }
  };
}, ub = (Z = {}) => {
  if (typeof Z == "string")
    return Z;
  const X = typeof window < "u" ? window.location.origin : "http://localhost", {
    endpoint: u = "/api/roll",
    dice: G,
    notation: s,
    roll: e,
    params: d = {},
    ...Y
  } = Z, c = new URL(u, X), l = (n, V, { append: a = !1 } = {}) => {
    if (!(V == null || V === "")) {
      if (Array.isArray(V)) {
        a || c.searchParams.delete(n), V.forEach((t) => l(n, t, { append: !0 }));
        return;
      }
      a ? c.searchParams.append(n, String(V)) : c.searchParams.set(n, String(V));
    }
  };
  if (Array.isArray(G))
    c.searchParams.delete("notation"), c.searchParams.delete("roll"), c.searchParams.delete("dice"), G.forEach((n) => l("dice", n, { append: !0 }));
  else {
    const n = s || e || G;
    n && c.searchParams.delete("dice"), l("notation", n);
  }
  return Object.entries({ ...Y, ...d }).forEach(([n, V]) => l(n, V)), typeof window < "u" && c.origin === window.location.origin ? `${c.pathname}${c.search}` : c.toString();
};
var el, Yl, yl, fl, al, Q, vl, Ll, _, tl, Nl, ml, Dl, ob, Al, hb, Ol, pb, sl, pl, Sl, pd, Fl, Jd, rl, yd, Hl, fd;
const ql = class ql {
  constructor(X = {}) {
    // Load the BabylonJS World
    j(this, Dl);
    // Load the AmmoJS physics world
    j(this, Al);
    j(this, Ol);
    j(this, sl);
    j(this, Sl);
    j(this, Fl);
    // used by both .add and .roll - .roll clears the box and .add does not
    j(this, rl);
    j(this, Hl);
    Wl(this, "rollCollectionData", {});
    Wl(this, "rollGroupData", {});
    Wl(this, "rollDiceData", {});
    Wl(this, "themeData", []);
    Wl(this, "themesLoadedData", {});
    j(this, el, 0);
    j(this, Yl, 0);
    j(this, yl, 0);
    j(this, fl, 0);
    j(this, al, null);
    j(this, Q, {});
    j(this, vl, void 0);
    j(this, Ll, void 0);
    j(this, _, void 0);
    j(this, tl, void 0);
    j(this, Nl, void 0);
    j(this, ml, !0);
    Wl(this, "noop", () => {
    });
    if (arguments.length === 2 && typeof (arguments[0] === "string") && typeof (arguments[1] === "object") && (console.warn("You are using the old API. Dicebox constructor accepts a config object as it's only argument. Please read the v1.1.0 docs at https://fantasticdice.games/docs/usage/config"), X = arguments[1], X.container = arguments[0]), typeof X != "object")
      throw new Error("Config options should be an object. Config reference: https://fantasticdice.games/docs/usage/config#configuration-options");
    const { onCollision: u, onBeforeRoll: G, onDieComplete: s, onRollComplete: e, onRemoveComplete: d, onThemeConfigLoaded: Y, onThemeLoaded: c, ...l } = X;
    this.config = { ...tX, ...l }, this.onBeforeRoll = X.onBeforeRoll || this.noop, this.onDieComplete = X.onDieComplete || this.noop, this.onRollComplete = X.onRollComplete || this.noop, this.onRemoveComplete = X.onRemoveComplete || this.noop, this.onThemeLoaded = X.onThemeLoaded || this.noop, this.onThemeConfigLoaded = X.onThemeConfigLoaded || this.noop, this.onCollision = X.onCollision || this.noop, Db() ? (this.canvas = jb({
      selector: this.config.container,
      id: this.config.id
    }), this.isVisible = !0) : D(this, ml, !1), this.loadThemeQueue = Tb();
  }
  static async fetchRollTrace(X = {}, u = {}) {
    const G = await fetch(ub(X), u), s = await G.json();
    if (!G.ok)
      throw new Error(s.error || G.statusText);
    return s;
  }
  resizeWorld() {
    const u = Ib(() => {
      F(this, Q).resize({ width: this.canvas.clientWidth, height: this.canvas.clientHeight }), F(this, _) && F(this, _).postMessage({ action: "resize", width: this.canvas.clientWidth, height: this.canvas.clientHeight });
    });
    window.addEventListener("resize", u);
  }
  async init() {
    return F(this, ml) ? $(this, Al, hb).call(this) : D(this, tl, Promise.resolve()), await $(this, Dl, ob).call(this), this.resizeWorld(), F(this, Q).onRollResult = (X) => {
      const u = this.rollDiceData[X.rollId], G = this.rollGroupData[u.groupId], s = this.rollCollectionData[u.collectionId];
      G.rolls[u.rollId].value = X.value, s.completedRolls++, s.completedRolls == s.rolls.length && s.resolve(Object.values(s.rolls).map(({ collectionId: c, id: l, meshName: n, ...V }) => V));
      const { collectionId: e, id: d, ...Y } = u;
      this.onDieComplete(Y);
    }, F(this, Q).onRollComplete = () => {
      this.onRollComplete(this.getRollResults());
    }, F(this, Q).onDieRemoved = (X) => {
      let u = this.rollDiceData[X];
      const G = this.rollCollectionData[u.removeCollectionId];
      G.completedRolls++, delete this.rollDiceData[u.rollId];
      const s = this.rollGroupData[u.groupId];
      delete s.rolls[u.rollId];
      const e = $(this, Hl, fd).call(this, u.groupId);
      s.value = e.value, s.qty = e.rollsArray.length, G.completedRolls == G.rolls.length && G.resolve(Object.values(G.rolls).map(({ id: V, ...a }) => a));
      const { collectionId: d, id: Y, removeCollectionId: c, meshName: l, ...n } = u;
      this.onRemoveComplete(n);
    }, await Promise.all([F(this, vl), F(this, tl)]), F(this, _) && $(this, Ol, pb).call(this), await this.loadThemeQueue.push(() => this.loadTheme(this.config.theme)), this.config.preloadThemes.forEach((async function(X) {
      await this.loadThemeQueue.push(() => this.loadTheme(X));
    }).bind(this)), this;
  }
  // fetch the theme config and return a themeData object
  async getThemeConfig(X) {
    let u = `${this.config.origin}${this.config.assetPath}themes/${X}`;
    this.config.externalThemes[X] && (u = this.config.externalThemes[X]);
    let G = await fetch(`${u}/theme.config.json`).then((d) => {
      if (d.ok) {
        const Y = d.headers.get("content-type");
        if (Y && Y.indexOf("application/json") !== -1)
          return d.json();
        if (d.type && d.type === "basic")
          return d.json();
        throw new Error(`Incorrect contentType: ${Y}. Expected "application/json" or "basic"`);
      } else
        throw new Error(`Unable to fetch config file for theme: '${X}'. Request rejected with status ${d.status}: ${d.statusText}`);
    }).catch((d) => console.error(d));
    if (!G)
      throw new Error("No theme config data to work with.");
    let s = "default", e = `${this.config.origin}${this.config.assetPath}themes/default/default.json`;
    if (G.hasOwnProperty("meshFile") && (s = G.meshFile.replace(/(.*)\..{2,4}$/, "$1"), e = `${u}/${G.meshFile}`), !G.hasOwnProperty("diceAvailable"))
      throw new Error('A theme must indicate which dice are available by defining "diceAvailable".');
    if (G.hasOwnProperty("extends")) {
      const d = await this.loadTheme(G.extends).catch((c) => console.error(c));
      if (d.hasOwnProperty("extends"))
        throw new Error("Cannot extend a theme that extends another theme.");
      const Y = {};
      G.diceAvailable.forEach((c) => {
        Y[c] = G.systemName;
      }), d.diceExtended = { ...d.diceExtended, ...Y }, this.config.theme = G.extends;
    }
    return Object.assign(
      G,
      {
        basePath: u,
        meshFilePath: e,
        meshName: s,
        theme: X
      }
    ), G;
  }
  async loadTheme(X) {
    if (this.themesLoadedData[X])
      return this.themesLoadedData[X];
    const u = this.themesLoadedData[X] = await this.getThemeConfig(X).catch((G) => console.error(G));
    if (this.onThemeConfigLoaded(u), !!u)
      return await F(this, Q).loadTheme(u).catch((G) => console.error(G)), this.onThemeLoaded(u), u;
  }
  // TODO: use getter and setter
  // change config options
  async updateConfig(X) {
    const u = { ...this.config, ...X };
    if (this.config = u, u.theme) {
      const s = (await this.loadThemeQueue.push(() => this.loadTheme(u.theme))).at(-1);
      s.hasOwnProperty("extends") && (this.config.theme = s.extends);
    }
    return F(this, Q).updateConfig(u), F(this, _) && F(this, _).postMessage({
      action: "updateConfig",
      options: u
    }), this;
  }
  clear() {
    return D(this, el, 0), D(this, Yl, 0), D(this, yl, 0), D(this, fl, 0), this.rollCollectionData = {}, this.rollGroupData = {}, this.rollDiceData = {}, F(this, Q).clear(), F(this, _) && F(this, _).postMessage({ action: "clearDice" }), this;
  }
  async replay(X, { speed: u = 1, onComplete: G } = {}) {
    var n;
    const s = typeof X == "string" ? JSON.parse(X) : X, e = s.metadata || s, d = oX(s.frames || s.frameData);
    if (!((n = e == null ? void 0 : e.renderDice) != null && n.length) || !(e != null && e.frame))
      throw new Error("Replay payload is missing metadata.renderDice or metadata.frame.");
    if (typeof F(this, Q).replay != "function")
      throw new Error("The active DiceBox world does not support replay.");
    this.clear();
    const Y = [...new Set(e.renderDice.map((V) => V.theme || e.theme || this.config.theme))];
    for (const V of Y)
      await this.loadThemeQueue.push(() => this.loadTheme(V));
    const l = await F(this, Q).replay({
      metadata: e,
      frameData: d,
      speed: u
    }) || e.results;
    return G && G(l), l;
  }
  async rollTrace(X, { frameRate: u = 60, ...G } = {}) {
    if (typeof F(this, Q).startRecording != "function" || typeof F(this, Q).stopRecording != "function")
      throw new Error("The active DiceBox world does not support roll tracing.");
    F(this, Q).startRecording({ frameRate: u });
    try {
      await this.roll(X, G);
      const s = this.getRollResults(), e = $(this, Fl, Jd).call(this, F(this, al), s), d = await F(this, Q).stopRecording();
      return pX({
        notation: X,
        results: s,
        finalResult: e,
        recording: d,
        config: this.config,
        canvas: this.canvas,
        theme: G.theme || this.config.theme,
        themeColor: G.themeColor || this.config.themeColor
      });
    } catch (s) {
      throw await F(this, Q).stopRecording(), s;
    }
  }
  fetchRollTrace(X = {}, u = {}) {
    return ql.fetchRollTrace(X, u);
  }
  async replayFromApi(X = {}, u = {}) {
    const G = await this.fetchRollTrace(X), s = await this.replay(G, u);
    return { trace: G, results: s };
  }
  hide(X) {
    return X ? (this.canvas.dataset.hideClass = X, this.canvas.classList.add(X)) : this.canvas.style.display = "none", this.isVisible = !1, this;
  }
  show() {
    var u;
    const X = (u = this.canvas.dataset) == null ? void 0 : u.hideClass;
    return X ? (delete this.canvas.dataset.hideClass, this.canvas.classList.remove(X)) : this.canvas.style.display = "block", this.isVisible = !0, this.resizeWorld(), this;
  }
  // TODO: pass data with roll - such as roll name. Passed back at the end in the results
  roll(X, { theme: u = this.config.theme, themeColor: G = this.config.themeColor, newStartPoint: s = !0 } = {}) {
    this.clear();
    const e = nl(this, el)._++;
    this.rollCollectionData[e] = new Yd({
      id: e,
      notation: X,
      theme: u,
      themeColor: G,
      newStartPoint: s
    }), D(this, al, e);
    const d = $(this, Sl, pd).call(this, X, e, this.themesLoadedData[u].diceAvailable);
    return $(this, rl, yd).call(this, d, e), this.rollCollectionData[e].promise;
  }
  add(X, { theme: u = this.config.theme, themeColor: G = this.config.themeColor, newStartPoint: s = !0 } = {}) {
    const e = nl(this, el)._++;
    this.rollCollectionData[e] = new Yd({
      id: e,
      notation: X,
      theme: u,
      themeColor: G,
      newStartPoint: s
    }), D(this, al, e);
    const d = $(this, Sl, pd).call(this, X, e, this.themesLoadedData[u].diceAvailable);
    return $(this, rl, yd).call(this, d, e), this.rollCollectionData[e].promise;
  }
  reroll(X, { remove: u = !1, hide: G = !1, newStartPoint: s = !0 } = {}) {
    const d = (Array.isArray(X) ? X : [X]).map(({ value: Y, ...c }) => c);
    return u === !0 && this.remove(d, { hide: G }), this.add(d, { newStartPoint: s });
  }
  remove(X, { hide: u = !1 } = {}) {
    const G = Array.isArray(X) ? X : [X], s = nl(this, el)._++;
    return this.rollCollectionData[s] = new Yd({
      id: s,
      notation: X,
      rolls: G
    }), G.map((e) => {
      this.rollDiceData[e.rollId].removeCollectionId = s;
      let d = this.rollDiceData[e.rollId].id;
      F(this, Q).remove({ id: d, rollId: e.rollId }), F(this, _) && F(this, _).postMessage({ action: "removeDie", id: d });
    }), this.rollCollectionData[s].promise;
  }
  // accepts simple notations eg: 4d6
  // accepts array of notations eg: ['4d6','2d10']
  // accepts object {sides:int, qty:int}
  // accepts array of objects eg: [{sides:int, qty:int, mods:[]}]
  createNotationArray(X, u) {
    if (typeof X == "string")
      return Ml(X, u, $(this, sl, pl).call(this)).diceGroups;
    const G = Array.isArray(X) ? X : [X];
    let s = [];
    const e = (c) => {
      if (c.hasOwnProperty("qty") || (c.qty = 1), c.hasOwnProperty("sides"))
        return c.sides === "100" && (c.sides = 100, c.data = "single"), !0;
      {
        const l = "Roll notation is missing sides";
        throw new Error(l);
      }
    }, d = (c) => {
      c = c.toString();
      let l = c.split(".");
      return l[1] ? l[1] = parseInt(l[1]) + 1 : l[1] = 1, l[0] + "." + l[1];
    }, Y = (c) => {
      c.hasOwnProperty("rollId") && this.rollDiceData.hasOwnProperty(c.rollId) && (c.rollId = d(c.rollId)), c.hasOwnProperty("modifier") || (c.modifier = 0);
    };
    return G.forEach((c) => {
      typeof c == "string" ? s.push(...Ml(c, u, $(this, sl, pl).call(this)).diceGroups) : typeof G == "object" && (Y(c), e(c) && s.push(c));
    }), s;
  }
  // parse text die notation such as 2d10+3 => {qty:2, sides:'d10', modifier:3}
  parse(X, u) {
    const G = Ml(X, u, $(this, sl, pl).call(this)).diceGroups;
    if (G.length !== 1)
      throw new Error("Roll notation expands to multiple dice groups. Use createNotationArray() for compound notation.");
    return G[0];
  }
  getRollResults() {
    const X = Object.entries(this.rollGroupData).map(([G, s]) => {
      const e = $(this, Hl, fd).call(this, G);
      s.value = e.value, s.qty = e.rollsArray.length;
      const d = { ...s };
      return d.rolls = e.rollsArray, d;
    });
    let u = null;
    try {
      u = $(this, Fl, Jd).call(this, F(this, al), X);
    } catch {
      u = null;
    }
    return u && Object.defineProperty(X, "finalResult", {
      value: u,
      enumerable: !1,
      configurable: !0
    }), X;
  }
};
el = new WeakMap(), Yl = new WeakMap(), yl = new WeakMap(), fl = new WeakMap(), al = new WeakMap(), Q = new WeakMap(), vl = new WeakMap(), Ll = new WeakMap(), _ = new WeakMap(), tl = new WeakMap(), Nl = new WeakMap(), ml = new WeakMap(), Dl = new WeakSet(), ob = async function() {
  D(this, vl, new Promise((u, G) => {
    D(this, Ll, u);
  }));
  const X = () => {
    F(this, Ll).call(this);
  };
  if (F(this, ml))
    if ("OffscreenCanvas" in window && "transferControlToOffscreen" in this.canvas && this.config.offscreen) {
      const u = await import("./world.offscreen.js").then((G) => G.default);
      D(this, Q, new u({
        canvas: this.canvas,
        options: this.config,
        onInitComplete: X
      }));
    } else {
      this.config.offscreen && (console.warn("This browser does not support OffscreenCanvas. Using standard canvas fallback."), this.config.offscreen = !1);
      const u = await import("./world.onscreen.js").then((G) => G.default);
      D(this, Q, new u({
        canvas: this.canvas,
        options: this.config,
        onInitComplete: X
      }));
    }
  else {
    console.warn("This browser does not support WebGL which is required for 3D rendering. Falling back to random number generator");
    const u = await import("./world.none.js").then((G) => G.default);
    D(this, Q, new u({
      canvas: this.canvas,
      options: this.config,
      onInitComplete: X
    }));
  }
}, Al = new WeakSet(), hb = function() {
  D(this, _, new Eb()), D(this, tl, new Promise((X, u) => {
    D(this, Nl, X);
  })), F(this, _).onmessage = (X) => {
    switch (X.data.action) {
      case "init-complete":
        F(this, Nl).call(this);
        break;
      case "collision":
        this.onCollision && this.onCollision(X.data.body0Id, X.data.body1Id, X.data.force);
        break;
    }
  }, F(this, _).postMessage({
    action: "init",
    width: this.canvas.clientWidth,
    height: this.canvas.clientHeight,
    options: this.config
  });
}, Ol = new WeakSet(), pb = function() {
  const X = new MessageChannel();
  F(this, Q).connect(X.port1), F(this, _).postMessage({
    action: "connect"
  }, [X.port2]);
}, sl = new WeakSet(), pl = function() {
  return {
    maxNotationLength: this.config.maxNotationLength,
    maxDiceCount: this.config.maxDiceCount,
    maxSides: this.config.maxSides,
    maxRerollDepth: this.config.maxRerollDepth
  };
}, Sl = new WeakSet(), pd = function(X, u, G) {
  const s = this.rollCollectionData[u];
  if (typeof X == "string") {
    const d = Ml(X, G, $(this, sl, pl).call(this));
    return s.parsedRoll = d, d.diceGroups;
  }
  const e = this.createNotationArray(X, G);
  return s.parsedRoll = {
    originalNotation: Array.isArray(X) ? X.join(",") : "",
    parserNotation: null,
    diceGroups: e,
    parsedTree: null,
    mode: "legacy"
  }, e;
}, Fl = new WeakSet(), Jd = function(X, u) {
  const G = this.rollCollectionData[X];
  return G != null && G.parsedRoll ? YX(G.parsedRoll.parsedTree, u, {
    originalNotation: G.parsedRoll.originalNotation || G.notation,
    maxRerollDepth: this.config.maxRerollDepth
  }) : null;
}, rl = new WeakSet(), yd = async function(X, u) {
  this.onBeforeRoll(X);
  const G = this.rollCollectionData[u];
  let s = G.newStartPoint;
  X.forEach(async (e) => {
    var ll, ol, xl, Kl, Cl, wl;
    if (!e.sides)
      throw new Error("Improper dice notation or unable to parse notation");
    let d = e.theme || G.theme || this.config.theme;
    const Y = e.themeColor || G.themeColor || this.config.themeColor, c = {}, l = e.groupId !== void 0;
    let n;
    const V = () => this.loadTheme(d);
    await this.loadThemeQueue.push(V);
    let a = this.themesLoadedData[d].meshName, t = (ll = this.themesLoadedData[d]) == null ? void 0 : ll.diceAvailable, L = this.themesLoadedData[d].diceExtended || {}, N = (xl = (ol = this.themesLoadedData[d]) == null ? void 0 : ol.material) == null ? void 0 : xl.type;
    const J = Object.keys(L);
    if (J && J.includes(e.sides)) {
      d = L[e.sides];
      const ul = () => this.loadTheme(d);
      this.loadThemeQueue.push(ul), a = this.themesLoadedData[d].meshName, t = (Kl = this.themesLoadedData[d]) == null ? void 0 : Kl.diceAvailable, N = (wl = (Cl = this.themesLoadedData[d]) == null ? void 0 : Cl.material) == null ? void 0 : wl.type;
    }
    let y = "", C;
    N === "color" && (C = Mb(Y), y = C.r * 0.299 + C.g * 0.587 + C.b * 0.114 > 175 ? "_dark" : "_light");
    for (var T = 0, E = e.qty; T < E; T++) {
      let ul = e.rollId !== void 0 ? e.rollId : nl(this, yl)._++, zl = e.id !== void 0 ? e.id : nl(this, fl)._++;
      n = l ? e.groupId : F(this, Yl);
      const Xl = Number.isInteger(e.sides) ? `d${e.sides}` : e.sides;
      /^d[1-9]{1}[0-9]{0,1}0?$/.test(e.sides) && (e.sides = parseInt(e.sides.replace("d", "")));
      const A = {
        sides: e.sides,
        data: e.data,
        dieType: Xl,
        groupId: n,
        collectionId: G.id,
        rollId: ul,
        id: zl,
        theme: d,
        themeColor: Y,
        meshName: a
      };
      if (c[ul] = A, this.rollDiceData[ul] = A, G.rolls.push(this.rollDiceData[ul]), A.sides === "fate" && !t.includes(Xl) && !J.includes(Xl) || A.sides === "fate" && !F(this, ml)) {
        console.warn(`fate die unavailable in '${d}' theme. Using fallback.`);
        const bl = -1, Gl = 1;
        A.value = bb.range(bl, Gl), F(this, Q).addNonDie(A);
      } else if (this.config.suspendSimulation || !t.includes(Xl) && !J.includes(Xl) || !F(this, ml)) {
        const bl = F(this, ml) ? this.config.suspendSimulation ? "3D simulation suspended. Using fallback." : `${A.sides} die unavailable in '${d}' theme. Using fallback.` : "This browser does not support webGL. Using random number fallback.";
        console.warn(bl);
        const Gl = Number.isInteger(A.sides) ? A.sides : parseInt(A.sides.replace(/\D/g, ""));
        A.value = bb.range(1, Gl), F(this, Q).addNonDie(A);
      } else {
        let bl;
        if (J.includes(Xl)) {
          const Gl = L[Xl];
          bl = this.themesLoadedData[Gl];
        }
        F(this, Q).add({
          ...A,
          newStartPoint: s,
          theme: (bl == null ? void 0 : bl.systemName) || d,
          meshName: (bl == null ? void 0 : bl.meshName) || a,
          colorSuffix: y
        });
      }
      s = !1;
    }
    l ? Object.assign(this.rollGroupData[n].rolls, c) : (e.rolls = c, e.id = n, this.rollGroupData[n] = e, ++nl(this, Yl)._);
  });
}, Hl = new WeakSet(), fd = function(X) {
  const u = this.rollGroupData[X], G = Object.values(u.rolls).map(({ collectionId: e, id: d, meshName: Y, ...c }) => c);
  let s = G.reduce((e, d) => {
    const Y = isNaN(d.value) ? 0 : d.value;
    return e + Y;
  }, 0);
  return s += u.modifier ? u.modifier : 0, { value: s, rollsArray: G };
}, Wl(ql, "createRollApiUrl", ub);
let Gb = ql;
class Yd {
  constructor(X) {
    Object.assign(this, X), this.rolls = X.rolls || [], this.completedRolls = 0;
    const u = this;
    this.promise = new Promise((G, s) => {
      u.resolve = G, u.reject = s;
    });
  }
}
export {
  yX as d,
  Gb as default
};
//# sourceMappingURL=dice-box.es.js.map

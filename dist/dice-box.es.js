var rb = Object.defineProperty;
var Kb = (m, X, u) => X in m ? rb(m, X, { enumerable: !0, configurable: !0, writable: !0, value: u }) : m[X] = u;
var Wl = (m, X, u) => (Kb(m, typeof X != "symbol" ? X + "" : X, u), u), Rd = (m, X, u) => {
  if (!X.has(m))
    throw TypeError("Cannot " + u);
};
var F = (m, X, u) => (Rd(m, X, "read from private field"), u ? u.call(m) : X.get(m)), j = (m, X, u) => {
  if (X.has(m))
    throw TypeError("Cannot add the same private member more than once");
  X instanceof WeakSet ? X.add(m) : X.set(m, u);
}, D = (m, X, u, G) => (Rd(m, X, "write to private field"), G ? G.call(m, u) : X.set(m, u), u);
var nl = (m, X, u, G) => ({
  set _(e) {
    D(m, X, e, u);
  },
  get _() {
    return F(m, X, G);
  }
}), $ = (m, X, u) => (Rd(m, X, "access private method"), u);
function Cb(m, X) {
  for (var u = 0; u < X.length; u++) {
    const G = X[u];
    if (typeof G != "string" && !Array.isArray(G)) {
      for (const e in G)
        if (e !== "default" && !(e in m)) {
          const a = Object.getOwnPropertyDescriptor(G, e);
          a && Object.defineProperty(m, e, a.get ? a : {
            enumerable: !0,
            get: () => G[e]
          });
        }
    }
  }
  return Object.freeze(Object.defineProperty(m, Symbol.toStringTag, { value: "Module" }));
}
function wb(m) {
  const { selector: X, id: u } = m;
  let G = document.body, e = document.createElement("canvas");
  if (e.id = u, e.classList.add("dice-box-canvas"), X) {
    if (typeof X != "string")
      throw new Error("You must provide a DOM selector as the first argument in order to render the Dice Box");
    if (G = document.querySelector(X), !(G != null && G.nodeName))
      throw new Error(`DiceBox target DOM node: '${X}' not found or not available yet. Try invoking inside a DOMContentLoaded event`);
  }
  return G.appendChild(e), e;
}
const Zb = "", _d = typeof window < "u" && window.Blob && new Blob([atob(Zb)], { type: "text/javascript;charset=utf-8" });
function zb() {
  let m;
  try {
    if (m = _d && (window.URL || window.webkitURL).createObjectURL(_d), !m)
      throw "";
    return new Worker(m);
  } catch {
    return new Worker("data:application/javascript;base64," + Zb);
  } finally {
    m && (window.URL || window.webkitURL).revokeObjectURL(m);
  }
}
const Bb = (m) => {
  let X;
  return function() {
    let u = this, G = arguments;
    X && window.cancelAnimationFrame(X), X = window.requestAnimationFrame(function() {
      m.apply(u, G);
    });
  };
}, Ub = (m = { dedupe: !1 }) => {
  const { dedupe: X } = m;
  let u = [], G;
  const e = (b) => (X && (u = []), u.push(b), G || (G = a()), G.finally(() => {
    G = void 0;
  })), a = async () => {
    const b = [];
    for (; u.length; ) {
      const t = u.shift();
      b.push(await t());
    }
    return b;
  };
  return { push: e, queue: u, flush: () => G || Promise.resolve([]) };
}, sX = (m) => JSON.parse(JSON.stringify(m));
class lb {
  /**
   * Generate a random number between 0 (inclusive) and 1 (exclusive).
   * A drop in replacement for Math.random()
   * @return {number}
   */
  static value() {
    const X = window.crypto || window.msCrypto, u = new Uint32Array(1);
    return X.getRandomValues(u)[0] / 2 ** 32;
  }
  /**
   * Generate a very good random number between min (inclusive) and max (exclusive) by using crypto.getRandomValues() twice.
   * @param  {number} min
   * @param  {number} max
   * @return {number}
   */
  static range(X, u) {
    return Math.floor(Math.pow(10, 14) * this.value() * this.value()) % (u - X + 1) + X;
  }
}
const kb = (m) => {
  let X = !1, u = m.slice(m.startsWith("#") ? 1 : 0);
  u.length === 3 ? u = [...u].map((e) => e + e).join("") : u.length === 8 && (X = !0), u = parseInt(u, 16);
  let G = {
    r: u >>> 16,
    g: (u & 65280) >>> 8,
    b: u & 255
  };
  return X && (G.r = u >>> 24, G.g = (u & 16711680) >>> 16, G.b = (u & 65280) >>> 8, G.a = u & 255), G;
};
function Qb() {
  try {
    const m = document.createElement("canvas");
    return !!window.WebGLRenderingContext && (m.getContext("webgl") || m.getContext("experimental-webgl"));
  } catch {
    return !1;
  }
}
var Pb = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function gb(m) {
  return m && m.__esModule && Object.prototype.hasOwnProperty.call(m, "default") ? m.default : m;
}
var mb = { exports: {} };
(function(m, X) {
  (function(u, G) {
    m.exports = G();
  })(Pb, () => (() => {
    var u = { 95: (e, a, b) => {
      Object.defineProperty(a, "__esModule", { value: !0 }), a.DiceRoller = void 0;
      const t = b(51);
      a.DiceRoller = class {
        constructor(c, l = 1e3) {
          this.randFunction = Math.random, this.maxRollCount = 1e3, c && (this.randFunction = c), this.maxRollCount = l;
        }
        parse(c) {
          return t.parse(c);
        }
        roll(c) {
          const l = t.parse(c);
          return this.rollType(l);
        }
        rollValue(c) {
          return this.roll(c).value;
        }
        rollParsed(c) {
          return this.rollType(c);
        }
        rollType(c) {
          let l;
          switch (c.type) {
            case "diceExpression":
              l = this.rollDiceExpr(c);
              break;
            case "group":
              l = this.rollGroup(c);
              break;
            case "die":
              l = this.rollDie(c);
              break;
            case "expression":
              l = this.rollExpression(c);
              break;
            case "mathfunction":
              l = this.rollFunction(c);
              break;
            case "inline":
              l = this.rollType(c.expr);
              break;
            case "number":
              l = Object.assign(Object.assign({}, c), { success: null, successes: 0, failures: 0, valid: !0, order: 0 });
              break;
            default:
              throw new Error(`Unable to render ${c.type}`);
          }
          return c.label && (l.label = c.label), l;
        }
        rollDiceExpr(c) {
          const l = this.rollType(c.head), n = [l], s = [], V = c.ops.reduce((Y, y, N) => {
            const J = this.rollType(y.tail);
            switch (J.order = N, n.push(J), s.push(y.op), y.op) {
              case "+":
                return Y + J.value;
              case "-":
                return Y - J.value;
              default:
                return Y;
            }
          }, l.value);
          return { dice: n, ops: s, success: null, successes: 0, failures: 0, type: "diceexpressionroll", valid: !0, value: V, order: 0 };
        }
        rollGroup(c) {
          let l = c.rolls.map((y, N) => Object.assign(Object.assign({}, this.rollType(y)), { order: N })), n = 0, s = 0, V = !1;
          if (c.mods) {
            const y = c.mods, N = (J) => (V = y.some((v) => ["failure", "success"].includes(v.type)), J = y.reduce((v, K) => this.applyGroupMod(v, K), J), V && (J = J.map((v) => (n += v.successes, s += v.failures, v.value = v.successes - v.failures, v.success = v.value > 0, v))), J);
            if (l.length === 1 && ["die", "diceexpressionroll"].includes(l[0].type)) {
              const J = l[0];
              let v = J.type === "die" ? J.rolls : J.dice.filter((K) => K.type !== "number").reduce((K, T) => [...K, ...T.type === "die" ? T.rolls : T.dice], []);
              v = N(v), J.value = v.reduce((K, T) => T.valid ? K + T.value : K, 0);
            } else
              l = N(l);
          }
          const Y = l.reduce((y, N) => N.valid ? y + N.value : y, 0);
          return { dice: l, success: V ? Y > 0 : null, successes: n, failures: s, type: "grouproll", valid: !0, value: Y, order: 0 };
        }
        rollDie(c) {
          const l = this.rollType(c.count);
          if (l.value > this.maxRollCount)
            throw new Error("Entered number of dice too large.");
          let n, s;
          c.die.type === "fate" ? (s = { type: "fate", success: null, successes: 0, failures: 0, valid: !1, value: 0, order: 0 }, n = Array.from({ length: l.value }, (v, K) => this.generateFateRoll(K))) : (s = this.rollType(c.die), n = Array.from({ length: l.value }, (v, K) => this.generateDiceRoll(s.value, K))), c.mods && (n = c.mods.reduce((v, K) => this.applyMod(v, K), n));
          let V = 0, Y = 0;
          c.targets && (n = c.targets.reduce((v, K) => this.applyMod(v, K), n).map((v) => (V += v.successes, Y += v.failures, v.value = v.successes - v.failures, v.success = v.value > 0, v)));
          let y = !1, N = 0;
          if (c.match) {
            const v = c.match, K = n.reduce((E, ll) => E.set(ll.roll, (E.get(ll.roll) || 0) + 1), /* @__PURE__ */ new Map()), T = new Set(Array.from(K.entries()).filter(([E, ll]) => ll >= v.min.value).filter(([E]) => !(v.mod && v.expr) || this.successTest(v.mod, this.rollType(v.expr).value, E)).map(([E]) => E));
            n.filter((E) => T.has(E.roll)).forEach((E) => E.matched = !0), v.count && (y = !0, N = T.size);
          }
          c.sort && (n = this.applySort(n, c.sort));
          const J = n.reduce((v, K) => K.valid ? v + K.value : v, 0);
          return { count: l, die: s, rolls: n, success: c.targets ? J > 0 : null, successes: V, failures: Y, type: "die", valid: !0, value: y ? N : J, order: 0, matched: y };
        }
        rollExpression(c) {
          const l = this.rollType(c.head), n = [l], s = [], V = c.ops.reduce((Y, y) => {
            const N = this.rollType(y.tail);
            switch (n.push(N), s.push(y.op), y.op) {
              case "+":
                return Y + N.value;
              case "-":
                return Y - N.value;
              case "*":
                return Y * N.value;
              case "/":
                return Y / N.value;
              case "%":
                return Y % N.value;
              case "**":
                return Y ** N.value;
              default:
                return Y;
            }
          }, l.value);
          return { dice: n, ops: s, success: null, successes: 0, failures: 0, type: "expressionroll", valid: !0, value: V, order: 0 };
        }
        rollFunction(c) {
          const l = this.rollType(c.expr);
          let n;
          switch (c.op) {
            case "floor":
              n = Math.floor(l.value);
              break;
            case "ceil":
              n = Math.ceil(l.value);
              break;
            case "round":
              n = Math.round(l.value);
              break;
            case "abs":
              n = Math.abs(l.value);
              break;
            default:
              n = l.value;
          }
          return { expr: l, op: c.op, success: null, successes: 0, failures: 0, type: "mathfunction", valid: !0, value: n, order: 0 };
        }
        applyGroupMod(c, l) {
          return this.getGroupModMethod(l)(c);
        }
        getGroupModMethod(c) {
          const l = (n) => n.value;
          switch (c.type) {
            case "success":
              return this.getSuccessMethod(c, l);
            case "failure":
              return this.getFailureMethod(c, l);
            case "keep":
              return this.getKeepMethod(c, l);
            case "drop":
              return this.getDropMethod(c, l);
            default:
              throw new Error(`Mod ${c.type} is not recognised`);
          }
        }
        applyMod(c, l) {
          return this.getModMethod(l)(c);
        }
        getModMethod(c) {
          const l = (n) => n.roll;
          switch (c.type) {
            case "success":
              return this.getSuccessMethod(c, l);
            case "failure":
              return this.getFailureMethod(c, l);
            case "crit":
              return this.getCritSuccessMethod(c, l);
            case "critfail":
              return this.getCritFailureMethod(c, l);
            case "keep":
              return (n) => this.getKeepMethod(c, l)(n).sort((s, V) => s.order - V.order);
            case "drop":
              return (n) => this.getDropMethod(c, l)(n).sort((s, V) => s.order - V.order);
            case "explode":
              return this.getExplodeMethod(c);
            case "compound":
              return this.getCompoundMethod(c);
            case "penetrate":
              return this.getPenetrateMethod(c);
            case "reroll":
              return this.getReRollMethod(c);
            case "rerollOnce":
              return this.getReRollOnceMethod(c);
            default:
              throw new Error(`Mod ${c.type} is not recognised`);
          }
        }
        applySort(c, l) {
          return c.sort((n, s) => l.asc ? n.roll - s.roll : s.roll - n.roll), c.forEach((n, s) => n.order = s), c;
        }
        getCritSuccessMethod(c, l) {
          const n = this.rollType(c.expr);
          return (s) => s.map((V) => {
            if (!V.valid || V.type !== "roll" || V.success)
              return V;
            const Y = V;
            return this.successTest(c.mod, n.value, l(V)) ? Y.critical = "success" : Y.critical === "success" && (Y.critical = null), V;
          });
        }
        getCritFailureMethod(c, l) {
          const n = this.rollType(c.expr);
          return (s) => s.map((V) => {
            if (!V.valid || V.type !== "roll" || V.success)
              return V;
            const Y = V;
            return this.successTest(c.mod, n.value, l(V)) ? Y.critical = "failure" : Y.critical === "failure" && (Y.critical = null), V;
          });
        }
        getSuccessMethod(c, l) {
          const n = this.rollType(c.expr);
          return (s) => s.map((V) => (V.valid && this.successTest(c.mod, n.value, l(V)) && (V.successes += 1), V));
        }
        getFailureMethod(c, l) {
          const n = this.rollType(c.expr);
          return (s) => s.map((V) => (V.valid && this.successTest(c.mod, n.value, l(V)) && (V.failures += 1), V));
        }
        getKeepMethod(c, l) {
          const n = this.rollType(c.expr);
          return (s) => {
            if (s.length === 0)
              return s;
            s = s.sort((J, v) => c.highlow === "l" ? l(v) - l(J) : l(J) - l(v)).sort((J, v) => (J.valid ? 1 : 0) - (v.valid ? 1 : 0));
            const V = Math.max(Math.min(n.value, s.length), 0);
            let Y = 0, y = 0;
            const N = s.reduce((J, v) => (v.valid ? 1 : 0) + J, 0) - V;
            for (; y < s.length && Y < N; )
              s[y].valid && (s[y].valid = !1, s[y].drop = !0, Y++), y++;
            return s;
          };
        }
        getDropMethod(c, l) {
          const n = this.rollType(c.expr);
          return (s) => {
            s = s.sort((N, J) => c.highlow === "h" ? l(J) - l(N) : l(N) - l(J));
            const V = Math.max(Math.min(n.value, s.length), 0);
            let Y = 0, y = 0;
            for (; y < s.length && Y < V; )
              s[y].valid && (s[y].valid = !1, s[y].drop = !0, Y++), y++;
            return s;
          };
        }
        getExplodeMethod(c) {
          const l = c.target ? this.rollType(c.target.value) : null;
          return (n) => {
            const s = l ? (V) => this.successTest(c.target.mod, l.value, V.roll) : (V) => this.successTest("=", V.type === "fateroll" ? 1 : V.die, V.roll);
            if (n[0].type === "roll" && s({ roll: 1 }) && s({ roll: n[0].die }))
              throw new Error("Invalid reroll target");
            for (let V = 0; V < n.length; V++) {
              let Y = n[V];
              Y.order = V;
              let y = 0;
              for (; s(Y) && y++ < 1e3; ) {
                Y.explode = !0;
                const N = this.reRoll(Y, ++V);
                n.splice(V, 0, N), Y = N;
              }
            }
            return n;
          };
        }
        getCompoundMethod(c) {
          const l = c.target ? this.rollType(c.target.value) : null;
          return (n) => {
            const s = l ? (V) => this.successTest(c.target.mod, l.value, V.roll) : (V) => this.successTest("=", V.type === "fateroll" ? 1 : V.die, V.roll);
            if (n[0].type === "roll" && s({ roll: 1 }) && s({ roll: n[0].die }))
              throw new Error("Invalid reroll target");
            for (let V = 0; V < n.length; V++) {
              let Y = n[V], y = Y.roll, N = 0;
              for (; s(Y) && N++ < 1e3; ) {
                Y.explode = !0;
                const J = this.reRoll(Y, V + 1);
                y += J.roll, Y = J;
              }
              n[V].value = y, n[V].roll = y;
            }
            return n;
          };
        }
        getPenetrateMethod(c) {
          const l = c.target ? this.rollType(c.target.value) : null;
          return (n) => {
            const s = l ? (V) => this.successTest(c.target.mod, l.value, V.roll) : (V) => this.successTest("=", V.type === "fateroll" ? 1 : V.die, V.roll);
            if (l && n[0].type === "roll" && s(n[0]) && this.successTest(c.target.mod, l.value, 1))
              throw new Error("Invalid reroll target");
            for (let V = 0; V < n.length; V++) {
              let Y = n[V];
              Y.order = V;
              let y = 0;
              for (; s(Y) && y++ < 1e3; ) {
                Y.explode = !0;
                const N = this.reRoll(Y, ++V);
                N.value -= 1, n.splice(V, 0, N), Y = N;
              }
            }
            return n;
          };
        }
        getReRollMethod(c) {
          const l = c.target ? this.successTest.bind(null, c.target.mod, this.rollType(c.target.value).value) : this.successTest.bind(null, "=", 1);
          return (n) => {
            if (n[0].type === "roll" && l(1) && l(n[0].die))
              throw new Error("Invalid reroll target");
            for (let s = 0; s < n.length; s++)
              for (; l(n[s].roll); ) {
                n[s].reroll = !0, n[s].valid = !1;
                const V = this.reRoll(n[s], s + 1);
                n.splice(++s, 0, V);
              }
            return n;
          };
        }
        getReRollOnceMethod(c) {
          const l = c.target ? this.successTest.bind(null, c.target.mod, this.rollType(c.target.value).value) : this.successTest.bind(null, "=", 1);
          return (n) => {
            if (n[0].type === "roll" && l(1) && l(n[0].die))
              throw new Error("Invalid reroll target");
            for (let s = 0; s < n.length; s++)
              if (l(n[s].roll)) {
                n[s].reroll = !0, n[s].valid = !1;
                const V = this.reRoll(n[s], s + 1);
                n.splice(++s, 0, V);
              }
            return n;
          };
        }
        successTest(c, l, n) {
          switch (c) {
            case ">":
              return n >= l;
            case "<":
              return n <= l;
            default:
              return n == l;
          }
        }
        reRoll(c, l) {
          switch (c.type) {
            case "roll":
              return this.generateDiceRoll(c.die, l);
            case "fateroll":
              return this.generateFateRoll(l);
            default:
              throw new Error(`Cannot do a reroll of a ${c.type}.`);
          }
        }
        generateDiceRoll(c, l) {
          const n = parseInt((this.randFunction() * c).toFixed(), 10) + 1;
          return { critical: n === c ? "success" : n === 1 ? "failure" : null, die: c, matched: !1, order: l, roll: n, success: null, successes: 0, failures: 0, type: "roll", valid: !0, value: n };
        }
        generateFateRoll(c) {
          const l = Math.floor(3 * this.randFunction()) - 1;
          return { matched: !1, order: c, roll: l, success: null, successes: 0, failures: 0, type: "fateroll", valid: !0, value: l };
        }
      };
    }, 619: (e, a) => {
      Object.defineProperty(a, "__esModule", { value: !0 }), a.DiscordRollRenderer = void 0, a.DiscordRollRenderer = class {
        render(b) {
          return this.doRender(b, !0);
        }
        doRender(b, t = !1) {
          let c = "";
          switch (b.type) {
            case "diceexpressionroll":
              c = this.renderGroupExpr(b);
              break;
            case "grouproll":
              c = this.renderGroup(b);
              break;
            case "die":
              c = this.renderDie(b);
              break;
            case "expressionroll":
              c = this.renderExpression(b);
              break;
            case "mathfunction":
              c = this.renderFunction(b);
              break;
            case "roll":
              return this.renderRoll(b);
            case "fateroll":
              return this.renderFateRoll(b);
            case "number":
              const l = b.label ? ` (${b.label})` : "";
              return `${b.value}${l}`;
            case "fate":
              return "F";
            default:
              throw new Error("Unable to render");
          }
          return b.valid || (c = "~~" + c.replace(/~~/g, "") + "~~"), t ? this.stripBrackets(c) : b.label ? `(${b.label}: ${c})` : c;
        }
        renderGroup(b) {
          const t = [];
          for (const c of b.dice)
            t.push(this.doRender(c));
          return t.length > 1 ? `{ ${t.join(" + ")} } = ${b.value}` : `{ ${this.stripBrackets(t[0])} } = ${b.value}`;
        }
        renderGroupExpr(b) {
          const t = [];
          for (const c of b.dice)
            t.push(this.doRender(c));
          return t.length > 1 ? `(${t.join(" + ")} = ${b.value})` : t[0];
        }
        renderDie(b) {
          const t = [];
          for (const n of b.rolls)
            t.push(this.doRender(n));
          let c = `${t.join(", ")}`;
          ["number", "fate"].includes(b.die.type) && b.count.type === "number" || (c += `[*Rolling: ${this.doRender(b.count)}d${this.doRender(b.die)}*]`);
          const l = b.matched ? " Match" + (b.value === 1 ? "" : "es") : "";
          return c += ` = ${b.value}${l}`, `(${c})`;
        }
        renderExpression(b) {
          if (b.dice.length > 1) {
            const t = [];
            for (let c = 0; c < b.dice.length - 1; c++)
              t.push(this.doRender(b.dice[c])), t.push(b.ops[c]);
            return t.push(this.doRender(b.dice.slice(-1)[0])), t.push("="), t.push(b.value + ""), `(${t.join(" ")})`;
          }
          return b.dice[0].type === "number" ? b.value + "" : this.doRender(b.dice[0]);
        }
        renderFunction(b) {
          const t = this.doRender(b.expr);
          return `(${b.op}${this.addBrackets(t)} = ${b.value})`;
        }
        addBrackets(b) {
          return b.startsWith("(") || (b = `(${b}`), b.endsWith(")") || (b = `${b})`), b;
        }
        stripBrackets(b) {
          return b.startsWith("(") && (b = b.substring(1)), b.endsWith(")") && (b = b.substring(0, b.length - 1)), b;
        }
        renderRoll(b) {
          let t = `${b.roll}`;
          return b.valid ? b.success && b.value === 1 ? t = `**${b.roll}**` : b.success && b.value === -1 ? t = `*${b.roll}*` : b.success || b.critical !== "success" ? b.success || b.critical !== "failure" || (t = `*${b.roll}*`) : t = `**${b.roll}**` : t = `~~${b.roll}~~`, b.matched && (t = `__${t}__`), t;
        }
        renderFateRoll(b) {
          const t = b.roll === 0 ? "0" : b.roll > 0 ? "+" : "-";
          let c = `${b.roll}`;
          return b.valid ? b.success && b.value === 1 ? c = `**${t}**` : b.success && b.value === -1 && (c = `*${t}*`) : c = `~~${t}~~`, b.matched && (c = `__${c}__`), c;
        }
      };
    }, 607: function(e, a, b) {
      var t = this && this.__createBinding || (Object.create ? function(l, n, s, V) {
        V === void 0 && (V = s);
        var Y = Object.getOwnPropertyDescriptor(n, s);
        Y && !("get" in Y ? !n.__esModule : Y.writable || Y.configurable) || (Y = { enumerable: !0, get: function() {
          return n[s];
        } }), Object.defineProperty(l, V, Y);
      } : function(l, n, s, V) {
        V === void 0 && (V = s), l[V] = n[s];
      }), c = this && this.__exportStar || function(l, n) {
        for (var s in l)
          s === "default" || Object.prototype.hasOwnProperty.call(n, s) || t(n, l, s);
      };
      Object.defineProperty(a, "__esModule", { value: !0 }), c(b(95), a), c(b(604), a), c(b(234), a), c(b(619), a), c(b(54), a);
    }, 604: (e, a) => {
      Object.defineProperty(a, "__esModule", { value: !0 });
    }, 234: (e, a) => {
      Object.defineProperty(a, "__esModule", { value: !0 });
    }, 54: (e, a) => {
      Object.defineProperty(a, "__esModule", { value: !0 });
    }, 51: (e) => {
      function a(b, t, c, l) {
        this.message = b, this.expected = t, this.found = c, this.location = l, this.name = "SyntaxError", typeof Error.captureStackTrace == "function" && Error.captureStackTrace(this, a);
      }
      (function(b, t) {
        function c() {
          this.constructor = b;
        }
        c.prototype = t.prototype, b.prototype = new c();
      })(a, Error), a.buildMessage = function(b, t) {
        var c = { literal: function(V) {
          return '"' + n(V.text) + '"';
        }, class: function(V) {
          var Y, y = "";
          for (Y = 0; Y < V.parts.length; Y++)
            y += V.parts[Y] instanceof Array ? s(V.parts[Y][0]) + "-" + s(V.parts[Y][1]) : s(V.parts[Y]);
          return "[" + (V.inverted ? "^" : "") + y + "]";
        }, any: function(V) {
          return "any character";
        }, end: function(V) {
          return "end of input";
        }, other: function(V) {
          return V.description;
        } };
        function l(V) {
          return V.charCodeAt(0).toString(16).toUpperCase();
        }
        function n(V) {
          return V.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/\0/g, "\\0").replace(/\t/g, "\\t").replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/[\x00-\x0F]/g, function(Y) {
            return "\\x0" + l(Y);
          }).replace(/[\x10-\x1F\x7F-\x9F]/g, function(Y) {
            return "\\x" + l(Y);
          });
        }
        function s(V) {
          return V.replace(/\\/g, "\\\\").replace(/\]/g, "\\]").replace(/\^/g, "\\^").replace(/-/g, "\\-").replace(/\0/g, "\\0").replace(/\t/g, "\\t").replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/[\x00-\x0F]/g, function(Y) {
            return "\\x0" + l(Y);
          }).replace(/[\x10-\x1F\x7F-\x9F]/g, function(Y) {
            return "\\x" + l(Y);
          });
        }
        return "Expected " + function(V) {
          var Y, y, N, J = new Array(V.length);
          for (Y = 0; Y < V.length; Y++)
            J[Y] = (N = V[Y], c[N.type](N));
          if (J.sort(), J.length > 0) {
            for (Y = 1, y = 1; Y < J.length; Y++)
              J[Y - 1] !== J[Y] && (J[y] = J[Y], y++);
            J.length = y;
          }
          switch (J.length) {
            case 1:
              return J[0];
            case 2:
              return J[0] + " or " + J[1];
            default:
              return J.slice(0, -1).join(", ") + ", or " + J[J.length - 1];
          }
        }(b) + " but " + function(V) {
          return V ? '"' + n(V) + '"' : "end of input";
        }(t) + " found.";
      }, e.exports = { SyntaxError: a, parse: function(b, t) {
        t = t !== void 0 ? t : {};
        var c, l = {}, n = { start: Pd }, s = Pd, V = { type: "any" }, Y = H("[[", !1), y = H("]]", !1), N = function(Z, W) {
          return W && (Z.label = W), Z;
        }, J = ">", v = H(">", !1), K = "<", T = H("<", !1), E = "=", ll = H("=", !1), tl = H("f", !1), rl = H("cs", !1), Kl = H("cf", !1), Cl = H("m", !1), wl = H("t", !1), ul = H("k", !1), zl = H("l", !1), Xl = H("h", !1), A = H("d", !1), bl = H("{", !1), Gl = H(",", !1), Rb = H("}", !1), Bl = "+", Ul = H("+", !1), eb = H("s", !1), sb = H("a", !1), Yb = H("!", !1), tb = H("!!", !1), hb = H("!p", !1), ob = H("r", !1), pb = H("ro", !1), Jb = H("F", !1), $l = H("%", !1), vd = H("(", !1), fd = H(")", !1), _l = H("-", !1), ld = function(Z, W) {
          return W.length == 0 ? Z : { head: Z, type: "expression", ops: W.map((i) => ({ type: "math", op: i[1], tail: i[3] })) };
        }, Ld = H("*", !1), Nd = H("/", !1), kl = "**", Sd = H("**", !1), Fd = "floor", yb = H("floor", !1), Hd = "ceil", vb = H("ceil", !1), xd = "round", fb = H("round", !1), Lb = H("abs", !1), Nb = Ud("integer"), rd = /^[0-9]/, Kd = bd([["0", "9"]], !1, !1), Sb = H("[", !1), Cd = /^[^\]]/, wd = bd(["]"], !0, !1), Fb = H("]", !1), Hb = Ud("whitespace"), zd = /^[ \t\n\r]/, Bd = bd([" ", "	", `
`, "\r"], !1, !1), d = 0, r = 0, Ql = [{ line: 1, column: 1 }], cl = 0, dd = [], h = 0;
        if ("startRule" in t) {
          if (!(t.startRule in n))
            throw new Error(`Can't start parsing from rule "` + t.startRule + '".');
          s = n[t.startRule];
        }
        function H(Z, W) {
          return { type: "literal", text: Z, ignoreCase: W };
        }
        function bd(Z, W, i) {
          return { type: "class", parts: Z, inverted: W, ignoreCase: i };
        }
        function Ud(Z) {
          return { type: "other", description: Z };
        }
        function kd(Z) {
          var W, i = Ql[Z];
          if (i)
            return i;
          for (W = Z - 1; !Ql[W]; )
            W--;
          for (i = { line: (i = Ql[W]).line, column: i.column }; W < Z; )
            b.charCodeAt(W) === 10 ? (i.line++, i.column = 1) : i.column++, W++;
          return Ql[Z] = i, i;
        }
        function Qd(Z, W) {
          var i = kd(Z), R = kd(W);
          return { start: { offset: Z, line: i.line, column: i.column }, end: { offset: W, line: R.line, column: R.column } };
        }
        function o(Z) {
          d < cl || (d > cl && (cl = d, dd = []), dd.push(Z));
        }
        function Pd() {
          var Z, W, i, R, f, p;
          if (Z = d, (W = Zd()) !== l) {
            for (i = [], b.length > d ? (R = b.charAt(d), d++) : (R = l, h === 0 && o(V)); R !== l; )
              i.push(R), b.length > d ? (R = b.charAt(d), d++) : (R = l, h === 0 && o(V));
            i !== l ? (r = Z, p = i, (f = W).root = !0, p && (f.label = p.join("")), Z = W = f) : (d = Z, Z = l);
          } else
            d = Z, Z = l;
          return Z;
        }
        function Pl() {
          var Z, W, i;
          return Z = d, b.charCodeAt(d) === 62 ? (W = J, d++) : (W = l, h === 0 && o(v)), W === l && (b.charCodeAt(d) === 60 ? (W = K, d++) : (W = l, h === 0 && o(T)), W === l && (b.charCodeAt(d) === 61 ? (W = E, d++) : (W = l, h === 0 && o(ll)))), W !== l && (i = Zl()) !== l ? (r = Z, Z = W = { type: "success", mod: W, expr: i }) : (d = Z, Z = l), Z;
        }
        function gl() {
          var Z, W, i, R;
          return Z = d, b.charCodeAt(d) === 102 ? (W = "f", d++) : (W = l, h === 0 && o(tl)), W !== l ? (b.charCodeAt(d) === 62 ? (i = J, d++) : (i = l, h === 0 && o(v)), i === l && (b.charCodeAt(d) === 60 ? (i = K, d++) : (i = l, h === 0 && o(T)), i === l && (b.charCodeAt(d) === 61 ? (i = E, d++) : (i = l, h === 0 && o(ll)))), i === l && (i = null), i !== l && (R = Zl()) !== l ? (r = Z, Z = W = { type: "failure", mod: i, expr: R }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function gd() {
          var Z, W, i, R;
          return Z = d, b.substr(d, 2) === "cs" ? (W = "cs", d += 2) : (W = l, h === 0 && o(rl)), W !== l ? (b.charCodeAt(d) === 62 ? (i = J, d++) : (i = l, h === 0 && o(v)), i === l && (b.charCodeAt(d) === 60 ? (i = K, d++) : (i = l, h === 0 && o(T)), i === l && (b.charCodeAt(d) === 61 ? (i = E, d++) : (i = l, h === 0 && o(ll)))), i === l && (i = null), i !== l && (R = Zl()) !== l ? (r = Z, Z = W = { type: "crit", mod: i, expr: R }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function jd() {
          var Z, W, i, R;
          return Z = d, b.substr(d, 2) === "cf" ? (W = "cf", d += 2) : (W = l, h === 0 && o(Kl)), W !== l ? (b.charCodeAt(d) === 62 ? (i = J, d++) : (i = l, h === 0 && o(v)), i === l && (b.charCodeAt(d) === 60 ? (i = K, d++) : (i = l, h === 0 && o(T)), i === l && (b.charCodeAt(d) === 61 ? (i = E, d++) : (i = l, h === 0 && o(ll)))), i === l && (i = null), i !== l && (R = Zl()) !== l ? (r = Z, Z = W = { type: "critfail", mod: i, expr: R }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function jl() {
          var Z, W, i, R;
          return Z = d, b.charCodeAt(d) === 107 ? (W = "k", d++) : (W = l, h === 0 && o(ul)), W !== l ? (b.charCodeAt(d) === 108 ? (i = "l", d++) : (i = l, h === 0 && o(zl)), i === l && (b.charCodeAt(d) === 104 ? (i = "h", d++) : (i = l, h === 0 && o(Xl))), i === l && (i = null), i !== l ? ((R = Zl()) === l && (R = null), R !== l ? (r = Z, Z = W = { type: "keep", highlow: i, expr: R || qd }) : (d = Z, Z = l)) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function El() {
          var Z, W, i, R;
          return Z = d, b.charCodeAt(d) === 100 ? (W = "d", d++) : (W = l, h === 0 && o(A)), W !== l ? (b.charCodeAt(d) === 108 ? (i = "l", d++) : (i = l, h === 0 && o(zl)), i === l && (b.charCodeAt(d) === 104 ? (i = "h", d++) : (i = l, h === 0 && o(Xl))), i === l && (i = null), i !== l ? ((R = Zl()) === l && (R = null), R !== l ? (r = Z, Z = W = { type: "drop", highlow: i, expr: R || qd }) : (d = Z, Z = l)) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function Xd() {
          var Z, W, i, R, f, p, L, x, S, B;
          if (Z = d, (W = cd()) !== l) {
            for (i = [], R = d, (f = z()) !== l ? (b.charCodeAt(d) === 43 ? (p = Bl, d++) : (p = l, h === 0 && o(Ul)), p !== l && (L = z()) !== l && (x = cd()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l); R !== l; )
              i.push(R), R = d, (f = z()) !== l ? (b.charCodeAt(d) === 43 ? (p = Bl, d++) : (p = l, h === 0 && o(Ul)), p !== l && (L = z()) !== l && (x = cd()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l);
            i !== l ? (r = Z, S = W, Z = W = (B = i).length == 0 ? S : { head: S, type: "diceExpression", ops: B.map((w) => ({ type: "math", op: w[1], tail: w[3] })) }) : (d = Z, Z = l);
          } else
            d = Z, Z = l;
          return Z;
        }
        function cd() {
          var Z;
          return (Z = Ed()) === l && (Z = Zd()), Z;
        }
        function Ed() {
          var Z, W, i;
          return Z = d, W = function() {
            var R, f, p, L, x;
            if (R = d, f = function() {
              var S, B, w, P, U, I;
              if (S = d, B = function() {
                var k, O, g, q;
                return k = d, (O = Zl()) === l && (O = null), O !== l ? (b.charCodeAt(d) === 100 ? (g = "d", d++) : (g = l, h === 0 && o(A)), g !== l ? (q = function() {
                  var dl, M;
                  return dl = d, b.charCodeAt(d) === 70 ? (M = "F", d++) : (M = l, h === 0 && o(Jb)), M === l && (b.charCodeAt(d) === 102 ? (M = "f", d++) : (M = l, h === 0 && o(tl))), M !== l && (r = dl, M = { type: "fate" }), M;
                }(), q === l && (q = function() {
                  var dl, M;
                  return dl = d, b.charCodeAt(d) === 37 ? (M = "%", d++) : (M = l, h === 0 && o($l)), M !== l && (r = dl, M = { type: "number", value: "100" }), M;
                }(), q === l && (q = Zl())), q !== l ? (r = k, k = O = { die: q, count: O || { type: "number", value: 1 }, type: "die" }) : (d = k, k = l)) : (d = k, k = l)) : (d = k, k = l), k;
              }(), B !== l) {
                for (w = [], (P = Td()) === l && (P = Md()) === l && (P = Id()) === l && (P = Ad()) === l && (P = Dd()); P !== l; )
                  w.push(P), (P = Td()) === l && (P = Md()) === l && (P = Id()) === l && (P = Ad()) === l && (P = Dd());
                w !== l ? (r = S, I = w, (U = B).mods = (U.mods || []).concat(I), S = B = U) : (d = S, S = l);
              } else
                d = S, S = l;
              return S;
            }(), f !== l) {
              for (p = [], (L = El()) === l && (L = jl()) === l && (L = Pl()) === l && (L = gl()) === l && (L = jd()) === l && (L = gd()); L !== l; )
                p.push(L), (L = El()) === l && (L = jl()) === l && (L = Pl()) === l && (L = gl()) === l && (L = jd()) === l && (L = gd());
              p !== l ? ((L = function() {
                var S, B, w, P, U;
                return S = d, b.charCodeAt(d) === 109 ? (B = "m", d++) : (B = l, h === 0 && o(Cl)), B !== l ? (b.charCodeAt(d) === 116 ? (w = "t", d++) : (w = l, h === 0 && o(wl)), w === l && (w = null), w !== l ? ((P = nd()) === l && (P = null), P !== l ? (U = function() {
                  var I, k, O;
                  return I = d, b.charCodeAt(d) === 62 ? (k = J, d++) : (k = l, h === 0 && o(v)), k === l && (b.charCodeAt(d) === 60 ? (k = K, d++) : (k = l, h === 0 && o(T)), k === l && (b.charCodeAt(d) === 61 ? (k = E, d++) : (k = l, h === 0 && o(ll)))), k !== l && (O = Zl()) !== l ? (r = I, I = k = { mod: k, expr: O }) : (d = I, I = l), I;
                }(), U === l && (U = null), U !== l ? (r = S, S = B = function(I, k, O) {
                  const g = { type: "match", min: k || { type: "number", value: 2 }, count: !!I };
                  return O && (g.mod = O.mod, g.expr = O.expr), g;
                }(w, P, U)) : (d = S, S = l)) : (d = S, S = l)) : (d = S, S = l)) : (d = S, S = l), S;
              }()) === l && (L = null), L !== l ? (x = function() {
                var S, B, w;
                return S = d, b.charCodeAt(d) === 115 ? (B = "s", d++) : (B = l, h === 0 && o(eb)), B !== l ? (b.charCodeAt(d) === 97 ? (w = "a", d++) : (w = l, h === 0 && o(sb)), w === l && (b.charCodeAt(d) === 100 ? (w = "d", d++) : (w = l, h === 0 && o(A))), w === l && (w = null), w !== l ? (r = S, S = B = w == "d" ? { type: "sort", asc: !1 } : { type: "sort", asc: !0 }) : (d = S, S = l)) : (d = S, S = l), S;
              }(), x === l && (x = null), x !== l ? (r = R, R = f = function(S, B, w, P) {
                const U = B.filter((I) => ["success", "failure"].includes(I.type));
                return B = B.filter((I) => !U.includes(I)), S.mods = (S.mods || []).concat(B), U.length > 0 && (S.targets = U), w && (S.match = w), P && (S.sort = P), S;
              }(f, p, L, x)) : (d = R, R = l)) : (d = R, R = l)) : (d = R, R = l);
            } else
              d = R, R = l;
            return R;
          }(), W !== l && z() !== l ? ((i = Il()) === l && (i = null), i !== l ? (r = Z, Z = W = N(W, i)) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function Id() {
          var Z, W, i;
          return Z = d, b.charCodeAt(d) === 33 ? (W = "!", d++) : (W = l, h === 0 && o(Yb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (r = Z, Z = W = { type: "explode", target: i }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function Td() {
          var Z, W, i;
          return Z = d, b.substr(d, 2) === "!!" ? (W = "!!", d += 2) : (W = l, h === 0 && o(tb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (r = Z, Z = W = { type: "compound", target: i }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function Md() {
          var Z, W, i;
          return Z = d, b.substr(d, 2) === "!p" ? (W = "!p", d += 2) : (W = l, h === 0 && o(hb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (r = Z, Z = W = { type: "penetrate", target: i }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function Dd() {
          var Z, W, i;
          return Z = d, b.charCodeAt(d) === 114 ? (W = "r", d++) : (W = l, h === 0 && o(ob)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (r = Z, Z = W = { type: "reroll", target: i || Od }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function Ad() {
          var Z, W, i;
          return Z = d, b.substr(d, 2) === "ro" ? (W = "ro", d += 2) : (W = l, h === 0 && o(pb)), W !== l ? ((i = hl()) === l && (i = null), i !== l ? (r = Z, Z = W = { type: "rerollOnce", target: i || Od }) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function hl() {
          var Z, W, i;
          return Z = d, b.charCodeAt(d) === 62 ? (W = J, d++) : (W = l, h === 0 && o(v)), W === l && (b.charCodeAt(d) === 60 ? (W = K, d++) : (W = l, h === 0 && o(T)), W === l && (b.charCodeAt(d) === 61 ? (W = E, d++) : (W = l, h === 0 && o(ll)))), W === l && (W = null), W !== l && (i = Zl()) !== l ? (r = Z, Z = W = { type: "target", mod: W, value: i }) : (d = Z, Z = l), Z;
        }
        function Zl() {
          var Z;
          return (Z = md()) === l && (Z = nd()), Z;
        }
        function Zd() {
          var Z;
          return (Z = function() {
            var W, i, R, f;
            return W = d, b.substr(d, 2) === "[[" ? (i = "[[", d += 2) : (i = l, h === 0 && o(Y)), i !== l && (R = Zd()) !== l ? (b.substr(d, 2) === "]]" ? (f = "]]", d += 2) : (f = l, h === 0 && o(y)), f !== l ? (r = W, W = i = { type: "inline", expr: R }) : (d = W, W = l)) : (d = W, W = l), W;
          }()) === l && (Z = ud()) === l && (Z = md()), Z;
        }
        function md() {
          var Z, W, i, R, f, p, L;
          return Z = d, b.charCodeAt(d) === 40 ? (W = "(", d++) : (W = l, h === 0 && o(vd)), W !== l && (i = ud()) !== l ? (b.charCodeAt(d) === 41 ? (R = ")", d++) : (R = l, h === 0 && o(fd)), R !== l && z() !== l ? ((f = Il()) === l && (f = null), f !== l ? (r = Z, p = i, (L = f) && (p.label = L), Z = W = p) : (d = Z, Z = l)) : (d = Z, Z = l)) : (d = Z, Z = l), Z;
        }
        function ud() {
          var Z, W, i, R, f, p, L, x;
          if (Z = d, (W = Gd()) !== l) {
            for (i = [], R = d, (f = z()) !== l ? (b.charCodeAt(d) === 43 ? (p = Bl, d++) : (p = l, h === 0 && o(Ul)), p === l && (b.charCodeAt(d) === 45 ? (p = "-", d++) : (p = l, h === 0 && o(_l))), p !== l && (L = z()) !== l && (x = Gd()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l); R !== l; )
              i.push(R), R = d, (f = z()) !== l ? (b.charCodeAt(d) === 43 ? (p = Bl, d++) : (p = l, h === 0 && o(Ul)), p === l && (b.charCodeAt(d) === 45 ? (p = "-", d++) : (p = l, h === 0 && o(_l))), p !== l && (L = z()) !== l && (x = Gd()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l);
            i !== l ? (r = Z, Z = W = ld(W, i)) : (d = Z, Z = l);
          } else
            d = Z, Z = l;
          return Z;
        }
        function Gd() {
          var Z, W, i, R, f, p, L, x;
          if (Z = d, (W = Wd()) !== l) {
            for (i = [], R = d, (f = z()) !== l ? (b.charCodeAt(d) === 42 ? (p = "*", d++) : (p = l, h === 0 && o(Ld)), p === l && (b.charCodeAt(d) === 47 ? (p = "/", d++) : (p = l, h === 0 && o(Nd))), p !== l && (L = z()) !== l && (x = Wd()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l); R !== l; )
              i.push(R), R = d, (f = z()) !== l ? (b.charCodeAt(d) === 42 ? (p = "*", d++) : (p = l, h === 0 && o(Ld)), p === l && (b.charCodeAt(d) === 47 ? (p = "/", d++) : (p = l, h === 0 && o(Nd))), p !== l && (L = z()) !== l && (x = Wd()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l);
            i !== l ? (r = Z, Z = W = ld(W, i)) : (d = Z, Z = l);
          } else
            d = Z, Z = l;
          return Z;
        }
        function Wd() {
          var Z, W, i, R, f, p, L, x;
          if (Z = d, (W = id()) !== l) {
            for (i = [], R = d, (f = z()) !== l ? (b.substr(d, 2) === kl ? (p = kl, d += 2) : (p = l, h === 0 && o(Sd)), p === l && (b.charCodeAt(d) === 37 ? (p = "%", d++) : (p = l, h === 0 && o($l))), p !== l && (L = z()) !== l && (x = id()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l); R !== l; )
              i.push(R), R = d, (f = z()) !== l ? (b.substr(d, 2) === kl ? (p = kl, d += 2) : (p = l, h === 0 && o(Sd)), p === l && (b.charCodeAt(d) === 37 ? (p = "%", d++) : (p = l, h === 0 && o($l))), p !== l && (L = z()) !== l && (x = id()) !== l ? R = f = [f, p, L, x] : (d = R, R = l)) : (d = R, R = l);
            i !== l ? (r = Z, Z = W = ld(W, i)) : (d = Z, Z = l);
          } else
            d = Z, Z = l;
          return Z;
        }
        function id() {
          var Z;
          return (Z = function() {
            var W, i, R, f, p;
            return W = d, i = function() {
              var L;
              return b.substr(d, 5) === Fd ? (L = Fd, d += 5) : (L = l, h === 0 && o(yb)), L === l && (b.substr(d, 4) === Hd ? (L = Hd, d += 4) : (L = l, h === 0 && o(vb)), L === l && (b.substr(d, 5) === xd ? (L = xd, d += 5) : (L = l, h === 0 && o(fb)), L === l && (b.substr(d, 3) === "abs" ? (L = "abs", d += 3) : (L = l, h === 0 && o(Lb))))), L;
            }(), i !== l && z() !== l ? (b.charCodeAt(d) === 40 ? (R = "(", d++) : (R = l, h === 0 && o(vd)), R !== l && z() !== l && (f = ud()) !== l && z() !== l ? (b.charCodeAt(d) === 41 ? (p = ")", d++) : (p = l, h === 0 && o(fd)), p !== l ? (r = W, W = i = { type: "mathfunction", op: i, expr: f }) : (d = W, W = l)) : (d = W, W = l)) : (d = W, W = l), W;
          }()) === l && (Z = function() {
            var W, i, R;
            return W = d, i = function() {
              var f, p, L, x, S, B, w, P;
              if (f = d, p = function() {
                var U, I, k, O, g, q, dl, M, Tl;
                if (U = d, b.charCodeAt(d) === 123 ? (I = "{", d++) : (I = l, h === 0 && o(bl)), I !== l)
                  if (z() !== l)
                    if ((k = Xd()) !== l) {
                      for (O = [], g = d, (q = z()) !== l ? (b.charCodeAt(d) === 44 ? (dl = ",", d++) : (dl = l, h === 0 && o(Gl)), dl !== l && (M = z()) !== l && (Tl = Xd()) !== l ? g = q = [q, dl, M, Tl] : (d = g, g = l)) : (d = g, g = l); g !== l; )
                        O.push(g), g = d, (q = z()) !== l ? (b.charCodeAt(d) === 44 ? (dl = ",", d++) : (dl = l, h === 0 && o(Gl)), dl !== l && (M = z()) !== l && (Tl = Xd()) !== l ? g = q = [q, dl, M, Tl] : (d = g, g = l)) : (d = g, g = l);
                      O !== l && (g = z()) !== l ? (b.charCodeAt(d) === 125 ? (q = "}", d++) : (q = l, h === 0 && o(Rb)), q !== l ? (r = U, U = I = { rolls: [k, ...O.map((xb) => xb[3])], type: "group" }) : (d = U, U = l)) : (d = U, U = l);
                    } else
                      d = U, U = l;
                  else
                    d = U, U = l;
                else
                  d = U, U = l;
                return U;
              }(), p !== l) {
                for (L = [], (x = jl()) === l && (x = El()) === l && (x = Pl()) === l && (x = gl()); x !== l; )
                  L.push(x), (x = jl()) === l && (x = El()) === l && (x = Pl()) === l && (x = gl());
                L !== l && (x = z()) !== l ? ((S = Il()) === l && (S = null), S !== l ? (r = f, B = p, P = S, (w = L).length > 0 && (B.mods = (B.mods || []).concat(w)), P && (B.label = P), f = p = B) : (d = f, f = l)) : (d = f, f = l);
              } else
                d = f, f = l;
              return f;
            }(), i === l && (i = Ed()) === l && (i = nd()), i !== l && z() !== l ? ((R = Il()) === l && (R = null), R !== l ? (r = W, W = i = N(i, R)) : (d = W, W = l)) : (d = W, W = l), W;
          }()) === l && (Z = md()), Z;
        }
        function nd() {
          var Z, W, i, R;
          if (h++, Z = d, b.charCodeAt(d) === 45 ? (W = "-", d++) : (W = l, h === 0 && o(_l)), W === l && (W = null), W !== l) {
            if (i = [], rd.test(b.charAt(d)) ? (R = b.charAt(d), d++) : (R = l, h === 0 && o(Kd)), R !== l)
              for (; R !== l; )
                i.push(R), rd.test(b.charAt(d)) ? (R = b.charAt(d), d++) : (R = l, h === 0 && o(Kd));
            else
              i = l;
            i !== l ? (r = Z, Z = W = { type: "number", value: parseInt(b.substring(r, d), 10) }) : (d = Z, Z = l);
          } else
            d = Z, Z = l;
          return h--, Z === l && (W = l, h === 0 && o(Nb)), Z;
        }
        function Il() {
          var Z, W, i, R;
          if (Z = d, b.charCodeAt(d) === 91 ? (W = "[", d++) : (W = l, h === 0 && o(Sb)), W !== l) {
            if (i = [], Cd.test(b.charAt(d)) ? (R = b.charAt(d), d++) : (R = l, h === 0 && o(wd)), R !== l)
              for (; R !== l; )
                i.push(R), Cd.test(b.charAt(d)) ? (R = b.charAt(d), d++) : (R = l, h === 0 && o(wd));
            else
              i = l;
            i !== l ? (b.charCodeAt(d) === 93 ? (R = "]", d++) : (R = l, h === 0 && o(Fb)), R !== l ? (r = Z, Z = W = i.join("")) : (d = Z, Z = l)) : (d = Z, Z = l);
          } else
            d = Z, Z = l;
          return Z;
        }
        function z() {
          var Z, W;
          for (h++, Z = [], zd.test(b.charAt(d)) ? (W = b.charAt(d), d++) : (W = l, h === 0 && o(Bd)); W !== l; )
            Z.push(W), zd.test(b.charAt(d)) ? (W = b.charAt(d), d++) : (W = l, h === 0 && o(Bd));
          return h--, Z === l && (W = l, h === 0 && o(Hb)), Z;
        }
        const Od = { type: "target", mod: "=", value: { type: "number", value: 1 } }, qd = { type: "number", value: 1 };
        if ((c = s()) !== l && d === b.length)
          return c;
        throw c !== l && d < b.length && o({ type: "end" }), Vd = dd, ad = cl < b.length ? b.charAt(cl) : null, $d = cl < b.length ? Qd(cl, cl + 1) : Qd(cl, cl), new a(a.buildMessage(Vd, ad), Vd, ad, $d);
        var Vd, ad, $d;
      } };
    } }, G = {};
    return function e(a) {
      var b = G[a];
      if (b !== void 0)
        return b.exports;
      var t = G[a] = { exports: {} };
      return u[a].call(t.exports, t, t.exports, e), t.exports;
    }(607);
  })());
})(mb);
var ub = mb.exports;
const Gb = /* @__PURE__ */ gb(ub), jb = /* @__PURE__ */ Cb({
  __proto__: null,
  default: Gb
}, [ub]), Eb = Gb || jb, { DiceRoller: Wb } = Eb, Ib = {
  maxNotationLength: 200,
  maxDiceCount: 100,
  maxSides: 100,
  maxRerollDepth: 100
}, Tb = /* @__PURE__ */ new Set(["explode", "compound", "penetrate", "reroll", "rerollOnce"]);
class Mb extends Error {
  constructor(X, u = 400) {
    super(X), this.name = "RollNotationError", this.statusCode = u;
  }
}
const C = (m) => new Mb(m), Jd = (m) => ({
  ...Ib,
  ...m
}), Db = (m) => {
  if (m !== void 0)
    return JSON.parse(JSON.stringify(m));
}, Ab = (m) => m.replace(/d00(?!\d)/gi, "d100").replace(/dFATE\b/gi, "dF"), Ob = (m) => {
  if (typeof m != "string")
    throw C("Roll notation must be a string.");
  const X = m.trim();
  if (!X)
    throw C("Roll notation cannot be empty.");
  return X;
}, qb = (m) => {
  const X = m.match(/@[A-Za-z_][\w.]*/u) || m.match(/@/u);
  if (X)
    throw C(`Unsupported roll notation contains unresolved app token '${X[0]}'. Resolve app tokens before rolling.`);
}, $b = (m, X) => {
  if (m.length > X.maxNotationLength)
    throw C(`Roll notation is too long. Maximum length is ${X.maxNotationLength} characters.`);
}, xl = (m) => m !== null && typeof m == "object", sd = (m) => xl(m) ? typeof m.label == "string" && m.label.trim() ? !0 : Object.values(m).some((X) => Array.isArray(X) ? X.some(sd) : sd(X)) : !1, Yd = (m) => xl(m) ? Tb.has(m.type) ? !0 : Object.values(m).some((X) => Array.isArray(X) ? X.some(Yd) : Yd(X)) : !1, il = (m) => xl(m) ? m.type === "die" ? !0 : Object.values(m).some((X) => Array.isArray(X) ? X.some(il) : il(X)) : !1, pl = (m, X) => {
  if (!xl(m))
    throw C(`Unsupported ${X}.`);
  switch (m.type) {
    case "number":
      return Number(m.value);
    case "expression": {
      let u = pl(m.head, X);
      for (const G of m.ops || []) {
        const e = pl(G.tail, X);
        switch (G.op) {
          case "+":
            u += e;
            break;
          case "-":
            u -= e;
            break;
          case "*":
            u *= e;
            break;
          case "/":
            u /= e;
            break;
          case "%":
            u %= e;
            break;
          case "**":
            u **= e;
            break;
          default:
            throw C(`Unsupported ${X} math operator '${G.op}'.`);
        }
      }
      return u;
    }
    case "mathfunction": {
      const u = pl(m.expr, X);
      switch (m.op) {
        case "floor":
          return Math.floor(u);
        case "ceil":
          return Math.ceil(u);
        case "round":
          return Math.round(u);
        case "abs":
          return Math.abs(u);
        default:
          throw C(`Unsupported ${X} math function '${m.op}'.`);
      }
    }
    default:
      throw C(`Unsupported ${X}. Dice count and sides must be static numbers.`);
  }
}, yd = (m, X) => {
  if (!Number.isInteger(m) || m < 1)
    throw C(`Invalid ${X}. Expected a positive integer.`);
  return m;
}, _b = (m, X) => {
  if ((m == null ? void 0 : m.type) === "fate")
    return "fate";
  const u = yd(pl(m, "die sides"), "die sides");
  if (u < 2)
    throw C("Invalid die sides. Dice must have at least 2 sides.");
  if (u > X.maxSides)
    throw C(`Roll notation exceeds the maximum die size of d${X.maxSides}.`);
  return u;
}, lX = (m, X) => `${m}d${X === "fate" ? "F" : X}`, dX = (m, X, u) => {
  const G = yd(pl(m.count, "dice count"), "dice count"), e = _b(m.die, X);
  return {
    qty: G,
    sides: e === "fate" ? "fate" : `d${e}`,
    modifier: 0,
    notation: lX(G, e),
    data: void 0,
    parserPath: u
  };
}, el = (m, X, u, G = "root") => {
  var e, a;
  if (xl(m))
    switch (m.type) {
      case "die":
        X.push(dX(m, u, G));
        for (const b of [...m.mods || [], ...m.targets || []]) {
          if (b.expr && il(b.expr))
            throw C("Unsupported roll notation. Modifier expressions cannot require additional physical dice.");
          if ((e = b.target) != null && e.value && il(b.target.value))
            throw C("Unsupported roll notation. Reroll target expressions cannot require additional physical dice.");
        }
        if ((a = m.match) != null && a.expr && il(m.match.expr))
          throw C("Unsupported roll notation. Match expressions cannot require additional physical dice.");
        break;
      case "diceExpression":
      case "expression":
        el(m.head, X, u, `${G}.head`);
        for (let b = 0; b < (m.ops || []).length; b++)
          el(m.ops[b].tail, X, u, `${G}.ops.${b}`);
        break;
      case "group":
        for (let b = 0; b < (m.rolls || []).length; b++)
          el(m.rolls[b], X, u, `${G}.rolls.${b}`);
        for (const b of m.mods || [])
          if (b.expr && il(b.expr))
            throw C("Unsupported roll notation. Group modifier expressions cannot require additional physical dice.");
        break;
      case "mathfunction":
        el(m.expr, X, u, `${G}.expr`);
        break;
      case "inline":
        el(m.expr, X, u, `${G}.expr`);
        break;
      case "number":
        break;
      default:
        throw C(`Unsupported roll notation node '${m.type}'.`);
    }
}, bX = (m) => m.reduce((X, u) => X + u.qty, 0), XX = (m) => {
  var u;
  if ((m == null ? void 0 : m.type) !== "expression" || il({ ...m, head: void 0 }) || !il(m.head))
    return 0;
  let X = 0;
  for (const G of m.ops || []) {
    if (((u = G.tail) == null ? void 0 : u.type) !== "number" || !["+", "-"].includes(G.op))
      return 0;
    X += G.op === "+" ? Number(G.tail.value) : -Number(G.tail.value);
  }
  return X;
}, cX = (m, X, u) => {
  if (m.length !== 1)
    return m;
  const G = XX(X);
  return G ? [{
    ...m[0],
    modifier: G,
    notation: u.replace(/\s+/g, "")
  }] : m;
}, ZX = (m, X) => {
  let u = (X.match(/[dD](?:00|%)(?!\d)/g) || []).length;
  return u ? m.map((G) => u > 0 && G.sides === "d100" ? (u--, { ...G, data: "single" }) : G) : m;
}, ib = (m, X) => {
  const u = bX(m);
  if (u < 1)
    throw C("Roll notation must include at least one physical die.");
  if (u > X.maxDiceCount)
    throw C(`Roll notation requests ${u} dice. Maximum dice count is ${X.maxDiceCount}.`);
  return m.forEach((G) => {
    yd(Number(G.qty), "dice count");
    const e = G.sides;
    if (e === "fate")
      return;
    const a = String(e).replace(/\D/g, "");
    if (!Number.isInteger(e) && !a && typeof e == "string")
      return;
    const b = Number.isInteger(e) ? e : parseInt(a, 10);
    if (!Number.isInteger(b) || b < 2)
      throw C("Invalid die sides. Dice must have at least 2 sides.");
    if (b > X.maxSides)
      throw C(`Roll notation exceeds the maximum die size of d${X.maxSides}.`);
  }), m;
}, mX = (m, X = {}) => {
  const u = Jd(X), G = Ob(m);
  $b(G, u), qb(G);
  const e = Ab(G), a = new Wb(null, u.maxRerollDepth);
  let b;
  try {
    b = a.parse(e);
  } catch (l) {
    throw C(`Invalid roll notation '${G}': ${l.message}`);
  }
  if (sd(b))
    throw C(`Unsupported roll notation '${G}': unexpected trailing text '${b.label.trim()}'.`);
  if (Yd(b) && !u.allowRerollingMods)
    throw C(`Reroll/explode notation is not yet supported in the replay API. Maximum configured reroll/explode depth is ${u.maxRerollDepth}.`);
  const t = [];
  el(b, t, u), ib(t, u);
  const c = cX(
    ZX(t, G),
    b,
    G
  );
  return {
    originalNotation: G,
    parserNotation: e,
    diceGroups: c,
    parsedTree: b,
    mode: "advanced"
  };
}, db = (m, X, u) => {
  const G = m === "" || m === void 0 ? X : Number(m);
  if (Number.isNaN(G) || !Number.isInteger(G) || G < 1)
    throw C(`Invalid notation: ${u}`);
  return G;
}, bb = (m, X = [], u = {}) => {
  const G = Jd(u), e = Array.isArray(m) ? m : String(m).split(",").map((n) => n.trim()).filter(Boolean), a = [], b = /^(\d*)[dD](00|%)([+-]\d+)?$/i, t = /^(\d*)[dD](f+[ate]*)([+-]\d+)?$/i, c = /^(\d*)[dD](\d+)([+-]\d+)?$/i, l = /^(\d*)[dD]([\d\w]+)([+-]\d+)?$/i;
  return e.forEach((n) => {
    if (typeof n == "object" && n !== null) {
      if (!n.sides)
        throw C("Roll notation is missing sides");
      a.push({ qty: 1, modifier: 0, ...n });
      return;
    }
    const s = String(n).trim().replace(/\s+/g, ""), V = s.match(b) || s.match(t) || s.match(c) || s.match(l);
    if (!V || V.length < 3)
      throw C(`Invalid notation: ${n}`);
    let Y = 0;
    V[3] && (Y = Number(V[3]));
    const y = {
      qty: db(V[1], 1, n),
      modifier: Y,
      notation: s
    };
    if (s.match(b))
      y.sides = "d100", y.data = "single";
    else if (s.match(t))
      y.sides = "fate";
    else if (s.match(c)) {
      const N = db(V[2], 1, n);
      y.sides = `d${N}`;
    } else if (X.includes(V[2]))
      y.sides = V[2];
    else
      throw C(`Invalid notation: ${n}`);
    a.push(y);
  }), ib(a, G), a;
}, Ml = (m, X = [], u = {}) => {
  if (typeof m == "string")
    try {
      return mX(m, u);
    } catch (G) {
      try {
        const e = bb(m, X, u);
        return {
          originalNotation: m.trim(),
          parserNotation: null,
          diceGroups: e,
          parsedTree: null,
          mode: "legacy",
          advancedError: G.message
        };
      } catch {
        throw G;
      }
    }
  return {
    originalNotation: Array.isArray(m) ? m.join(",") : "",
    parserNotation: null,
    diceGroups: bb(m, X, u),
    parsedTree: null,
    mode: "legacy"
  };
}, uX = (m) => {
  if (m.sides === "fate")
    return "fate";
  if (Number.isInteger(m.sides))
    return m.sides;
  if (typeof m.sides == "string") {
    const X = parseInt(m.sides.replace(/\D/g, ""), 10);
    if (Number.isInteger(X))
      return X;
  }
  if (typeof m.dieType == "string") {
    const X = parseInt(m.dieType.replace(/\D/g, ""), 10);
    if (Number.isInteger(X))
      return X;
  }
  throw C("Unable to determine die sides from physical roll results.");
}, GX = (m) => {
  const X = Number(m.value), u = uX(m);
  if (u === "fate") {
    if (![-1, 0, 1].includes(X))
      throw C(`Invalid fate die result '${m.value}'.`);
    return X === -1 ? 0 : X === 0 ? 1 / 3 : 2 / 3;
  }
  if (!Number.isInteger(X) || X < 1 || X > u)
    throw C(`Invalid d${u} physical result '${m.value}'.`);
  return (X - 1) / u;
}, WX = (m) => Array.isArray(m) ? m.flatMap((X) => Array.isArray(X.rolls) ? X.rolls : (X == null ? void 0 : X.value) !== void 0 ? [X] : []) : [], iX = (m, X, u = {}) => {
  const G = Jd(u), e = u.originalNotation || u.notation || "";
  if (!m)
    return {
      value: X.reduce((n, s) => n + (Number(s.value) || 0), 0),
      notation: e,
      type: "legacy",
      success: null,
      successes: 0,
      failures: 0
    };
  const a = WX(X);
  let b = 0;
  const c = new Wb(() => {
    if (!a[b])
      throw C("Parser requested more dice than were physically rolled.");
    return GX(a[b++]);
  }, G.maxRerollDepth).rollParsed(m);
  if (b !== a.length)
    throw C("Physical roll results did not match parsed dice notation.");
  return {
    value: c.value,
    notation: e,
    type: c.type,
    success: c.success ?? null,
    successes: c.successes || 0,
    failures: c.failures || 0,
    details: Db(c)
  };
}, nX = {
  id: `dice-canvas-${Date.now()}`,
  // set the canvas id
  container: null,
  enableShadows: !0,
  // do dice cast shadows onto DiceBox mesh?
  shadowTransparency: 0.8,
  lightIntensity: 1,
  delay: 10,
  // delay between dice being generated - 0 causes stuttering and physics popping
  scale: 5,
  // scale the dice
  theme: "default",
  // can be a hex color or a pre-defined theme such as 'purpleRock'
  preloadThemes: [],
  externalThemes: {},
  // point to CDN paths
  themeColor: "#2e8555",
  // used for color values or named theme variants - not fully implemented yet // green: #2e8555 // yellow: #feea03
  offscreen: !0,
  // use offscreen canvas browser feature for performance improvements - will fallback to false based on feature detection
  assetPath: "/assets/dice-box/",
  // path to 'ammo', 'themes' folders and web workers
  // origin: location.origin,
  origin: typeof window < "u" ? window.location.origin : "",
  suspendSimulation: !1,
  maxNotationLength: 200,
  maxDiceCount: 100,
  maxSides: 100,
  maxRerollDepth: 100
}, VX = (m) => {
  if (m instanceof Float32Array)
    return m;
  if (Array.isArray(m))
    return new Float32Array(m);
  if (m != null && m.data && Array.isArray(m.data))
    return new Float32Array(m.data);
  if ((m == null ? void 0 : m.type) === "Float32Array" && (m == null ? void 0 : m.encoding) === "base64") {
    let X;
    if (typeof atob == "function")
      X = atob(m.data);
    else if (typeof Buffer < "u") {
      const G = Buffer.from(m.data, "base64");
      return new Float32Array(G.buffer, G.byteOffset, G.byteLength / Float32Array.BYTES_PER_ELEMENT);
    }
    const u = new Uint8Array(X.length);
    for (let G = 0; G < X.length; G++)
      u[G] = X.charCodeAt(G);
    return new Float32Array(u.buffer);
  }
  throw new Error("Replay payload is missing Float32Array frame data.");
}, aX = (m) => {
  const X = new Uint8Array(m.buffer, m.byteOffset, m.byteLength);
  if (typeof Buffer < "u")
    return Buffer.from(X).toString("base64");
  let u = "";
  const G = 32768;
  for (let e = 0; e < X.length; e += G)
    u += String.fromCharCode(...X.subarray(e, e + G));
  return btoa(u);
}, RX = ({ notation: m, results: X, finalResult: u, recording: G, config: e, canvas: a, theme: b, themeColor: t }) => {
  var N;
  const c = G.frameData || new Float32Array(), l = G.frameRate || 60, n = ((N = G.renderDice) == null ? void 0 : N.length) || 0, s = 8, V = n ? c.length / (n * s) : 0, Y = X.flatMap((J) => J.rolls || []), y = {
    value: X.reduce((J, v) => J + (Number(v.value) || 0), 0),
    notation: Array.isArray(m) ? m.join(",") : String(m),
    type: "legacy",
    success: null,
    successes: 0,
    failures: 0
  };
  return {
    version: 1,
    metadata: {
      notation: Array.isArray(m) ? m.join(",") : String(m),
      generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
      seed: null,
      theme: b,
      themeColor: t,
      timedOut: !1,
      config: {
        width: (a == null ? void 0 : a.clientWidth) || (a == null ? void 0 : a.width),
        height: (a == null ? void 0 : a.clientHeight) || (a == null ? void 0 : a.height),
        scale: e.scale,
        frameRate: l,
        delay: e.delay,
        settleTimeout: e.settleTimeout
      },
      results: X,
      finalResult: u || y,
      rolls: Y,
      renderDice: G.renderDice || [],
      frame: {
        type: "Float32Array",
        fields: ["id", "px", "py", "pz", "qx", "qy", "qz", "qw"],
        stride: s,
        dieCount: n,
        frameCount: V,
        frameRate: l,
        length: c.length,
        durationMs: Math.round(V * (1e3 / l))
      }
    },
    frames: {
      type: "Float32Array",
      encoding: "base64",
      littleEndian: !0,
      length: c.length,
      data: aX(c)
    }
  };
}, Xb = (m = {}) => {
  if (typeof m == "string")
    return m;
  const X = typeof window < "u" ? window.location.origin : "http://localhost", {
    endpoint: u = "/api/roll",
    dice: G,
    notation: e,
    roll: a,
    params: b = {},
    ...t
  } = m, c = new URL(u, X), l = (n, s, { append: V = !1 } = {}) => {
    if (!(s == null || s === "")) {
      if (Array.isArray(s)) {
        V || c.searchParams.delete(n), s.forEach((Y) => l(n, Y, { append: !0 }));
        return;
      }
      V ? c.searchParams.append(n, String(s)) : c.searchParams.set(n, String(s));
    }
  };
  if (Array.isArray(G))
    c.searchParams.delete("notation"), c.searchParams.delete("roll"), c.searchParams.delete("dice"), G.forEach((n) => l("dice", n, { append: !0 }));
  else {
    const n = e || a || G;
    n && c.searchParams.delete("dice"), l("notation", n);
  }
  return Object.entries({ ...t, ...b }).forEach(([n, s]) => l(n, s)), typeof window < "u" && c.origin === window.location.origin ? `${c.pathname}${c.search}` : c.toString();
};
var Vl, sl, Jl, yl, al, Q, vl, fl, _, Yl, Ll, ml, Dl, nb, Al, Vb, Ol, ab, Rl, ol, Nl, td, Sl, hd, Fl, od, Hl, pd;
const ql = class ql {
  constructor(X = {}) {
    // Load the BabylonJS World
    j(this, Dl);
    // Load the AmmoJS physics world
    j(this, Al);
    j(this, Ol);
    j(this, Rl);
    j(this, Nl);
    j(this, Sl);
    // used by both .add and .roll - .roll clears the box and .add does not
    j(this, Fl);
    j(this, Hl);
    Wl(this, "rollCollectionData", {});
    Wl(this, "rollGroupData", {});
    Wl(this, "rollDiceData", {});
    Wl(this, "themeData", []);
    Wl(this, "themesLoadedData", {});
    j(this, Vl, 0);
    j(this, sl, 0);
    j(this, Jl, 0);
    j(this, yl, 0);
    j(this, al, null);
    j(this, Q, {});
    j(this, vl, void 0);
    j(this, fl, void 0);
    j(this, _, void 0);
    j(this, Yl, void 0);
    j(this, Ll, void 0);
    j(this, ml, !0);
    Wl(this, "noop", () => {
    });
    if (arguments.length === 2 && typeof (arguments[0] === "string") && typeof (arguments[1] === "object") && (console.warn("You are using the old API. Dicebox constructor accepts a config object as it's only argument. Please read the v1.1.0 docs at https://fantasticdice.games/docs/usage/config"), X = arguments[1], X.container = arguments[0]), typeof X != "object")
      throw new Error("Config options should be an object. Config reference: https://fantasticdice.games/docs/usage/config#configuration-options");
    const { onCollision: u, onBeforeRoll: G, onDieComplete: e, onRollComplete: a, onRemoveComplete: b, onThemeConfigLoaded: t, onThemeLoaded: c, ...l } = X;
    this.config = { ...nX, ...l }, this.onBeforeRoll = X.onBeforeRoll || this.noop, this.onDieComplete = X.onDieComplete || this.noop, this.onRollComplete = X.onRollComplete || this.noop, this.onRemoveComplete = X.onRemoveComplete || this.noop, this.onThemeLoaded = X.onThemeLoaded || this.noop, this.onThemeConfigLoaded = X.onThemeConfigLoaded || this.noop, this.onCollision = X.onCollision || this.noop, Qb() ? (this.canvas = wb({
      selector: this.config.container,
      id: this.config.id
    }), this.isVisible = !0) : D(this, ml, !1), this.loadThemeQueue = Ub();
  }
  static async fetchRollTrace(X = {}, u = {}) {
    const G = await fetch(Xb(X), u), e = await G.json();
    if (!G.ok)
      throw new Error(e.error || G.statusText);
    return e;
  }
  resizeWorld() {
    const u = Bb(() => {
      F(this, Q).resize({ width: this.canvas.clientWidth, height: this.canvas.clientHeight }), F(this, _) && F(this, _).postMessage({ action: "resize", width: this.canvas.clientWidth, height: this.canvas.clientHeight });
    });
    window.addEventListener("resize", u);
  }
  async init() {
    return F(this, ml) ? $(this, Al, Vb).call(this) : D(this, Yl, Promise.resolve()), await $(this, Dl, nb).call(this), this.resizeWorld(), F(this, Q).onRollResult = (X) => {
      const u = this.rollDiceData[X.rollId], G = this.rollGroupData[u.groupId], e = this.rollCollectionData[u.collectionId];
      G.rolls[u.rollId].value = X.value, e.completedRolls++, e.completedRolls == e.rolls.length && e.resolve(Object.values(e.rolls).map(({ collectionId: c, id: l, meshName: n, ...s }) => s));
      const { collectionId: a, id: b, ...t } = u;
      this.onDieComplete(t);
    }, F(this, Q).onRollComplete = () => {
      this.onRollComplete(this.getRollResults());
    }, F(this, Q).onDieRemoved = (X) => {
      let u = this.rollDiceData[X];
      const G = this.rollCollectionData[u.removeCollectionId];
      G.completedRolls++, delete this.rollDiceData[u.rollId];
      const e = this.rollGroupData[u.groupId];
      delete e.rolls[u.rollId];
      const a = $(this, Hl, pd).call(this, u.groupId);
      e.value = a.value, e.qty = a.rollsArray.length, G.completedRolls == G.rolls.length && G.resolve(Object.values(G.rolls).map(({ id: s, ...V }) => V));
      const { collectionId: b, id: t, removeCollectionId: c, meshName: l, ...n } = u;
      this.onRemoveComplete(n);
    }, await Promise.all([F(this, vl), F(this, Yl)]), F(this, _) && $(this, Ol, ab).call(this), await this.loadThemeQueue.push(() => this.loadTheme(this.config.theme)), this.config.preloadThemes.forEach((async function(X) {
      await this.loadThemeQueue.push(() => this.loadTheme(X));
    }).bind(this)), this;
  }
  // fetch the theme config and return a themeData object
  async getThemeConfig(X) {
    let u = `${this.config.origin}${this.config.assetPath}themes/${X}`;
    this.config.externalThemes[X] && (u = this.config.externalThemes[X]);
    let G = await fetch(`${u}/theme.config.json`).then((b) => {
      if (b.ok) {
        const t = b.headers.get("content-type");
        if (t && t.indexOf("application/json") !== -1)
          return b.json();
        if (b.type && b.type === "basic")
          return b.json();
        throw new Error(`Incorrect contentType: ${t}. Expected "application/json" or "basic"`);
      } else
        throw new Error(`Unable to fetch config file for theme: '${X}'. Request rejected with status ${b.status}: ${b.statusText}`);
    }).catch((b) => console.error(b));
    if (!G)
      throw new Error("No theme config data to work with.");
    let e = "default", a = `${this.config.origin}${this.config.assetPath}themes/default/default.json`;
    if (G.hasOwnProperty("meshFile") && (e = G.meshFile.replace(/(.*)\..{2,4}$/, "$1"), a = `${u}/${G.meshFile}`), !G.hasOwnProperty("diceAvailable"))
      throw new Error('A theme must indicate which dice are available by defining "diceAvailable".');
    if (G.hasOwnProperty("extends")) {
      const b = await this.loadTheme(G.extends).catch((c) => console.error(c));
      if (b.hasOwnProperty("extends"))
        throw new Error("Cannot extend a theme that extends another theme.");
      const t = {};
      G.diceAvailable.forEach((c) => {
        t[c] = G.systemName;
      }), b.diceExtended = { ...b.diceExtended, ...t }, this.config.theme = G.extends;
    }
    return Object.assign(
      G,
      {
        basePath: u,
        meshFilePath: a,
        meshName: e,
        theme: X
      }
    ), G;
  }
  async loadTheme(X) {
    if (this.themesLoadedData[X])
      return this.themesLoadedData[X];
    const u = this.themesLoadedData[X] = await this.getThemeConfig(X).catch((G) => console.error(G));
    if (this.onThemeConfigLoaded(u), !!u)
      return await F(this, Q).loadTheme(u).catch((G) => console.error(G)), this.onThemeLoaded(u), u;
  }
  // TODO: use getter and setter
  // change config options
  async updateConfig(X) {
    const u = { ...this.config, ...X };
    if (this.config = u, u.theme) {
      const e = (await this.loadThemeQueue.push(() => this.loadTheme(u.theme))).at(-1);
      e.hasOwnProperty("extends") && (this.config.theme = e.extends);
    }
    return F(this, Q).updateConfig(u), F(this, _) && F(this, _).postMessage({
      action: "updateConfig",
      options: u
    }), this;
  }
  clear() {
    return D(this, Vl, 0), D(this, sl, 0), D(this, Jl, 0), D(this, yl, 0), this.rollCollectionData = {}, this.rollGroupData = {}, this.rollDiceData = {}, F(this, Q).clear(), F(this, _) && F(this, _).postMessage({ action: "clearDice" }), this;
  }
  async replay(X, { speed: u = 1, onComplete: G } = {}) {
    var n;
    const e = typeof X == "string" ? JSON.parse(X) : X, a = e.metadata || e, b = VX(e.frames || e.frameData);
    if (!((n = a == null ? void 0 : a.renderDice) != null && n.length) || !(a != null && a.frame))
      throw new Error("Replay payload is missing metadata.renderDice or metadata.frame.");
    if (typeof F(this, Q).replay != "function")
      throw new Error("The active DiceBox world does not support replay.");
    this.clear();
    const t = [...new Set(a.renderDice.map((s) => s.theme || a.theme || this.config.theme))];
    for (const s of t)
      await this.loadThemeQueue.push(() => this.loadTheme(s));
    const l = await F(this, Q).replay({
      metadata: a,
      frameData: b,
      speed: u
    }) || a.results;
    return G && G(l), l;
  }
  async rollTrace(X, { frameRate: u = 60, ...G } = {}) {
    if (typeof F(this, Q).startRecording != "function" || typeof F(this, Q).stopRecording != "function")
      throw new Error("The active DiceBox world does not support roll tracing.");
    F(this, Q).startRecording({ frameRate: u });
    try {
      await this.roll(X, G);
      const e = this.getRollResults(), a = $(this, Sl, hd).call(this, F(this, al), e), b = await F(this, Q).stopRecording();
      return RX({
        notation: X,
        results: e,
        finalResult: a,
        recording: b,
        config: this.config,
        canvas: this.canvas,
        theme: G.theme || this.config.theme,
        themeColor: G.themeColor || this.config.themeColor
      });
    } catch (e) {
      throw await F(this, Q).stopRecording(), e;
    }
  }
  fetchRollTrace(X = {}, u = {}) {
    return ql.fetchRollTrace(X, u);
  }
  async replayFromApi(X = {}, u = {}) {
    const G = await this.fetchRollTrace(X), e = await this.replay(G, u);
    return { trace: G, results: e };
  }
  hide(X) {
    return X ? (this.canvas.dataset.hideClass = X, this.canvas.classList.add(X)) : this.canvas.style.display = "none", this.isVisible = !1, this;
  }
  show() {
    var u;
    const X = (u = this.canvas.dataset) == null ? void 0 : u.hideClass;
    return X ? (delete this.canvas.dataset.hideClass, this.canvas.classList.remove(X)) : this.canvas.style.display = "block", this.isVisible = !0, this.resizeWorld(), this;
  }
  // TODO: pass data with roll - such as roll name. Passed back at the end in the results
  roll(X, { theme: u = this.config.theme, themeColor: G = this.config.themeColor, newStartPoint: e = !0 } = {}) {
    this.clear();
    const a = nl(this, Vl)._++;
    this.rollCollectionData[a] = new ed({
      id: a,
      notation: X,
      theme: u,
      themeColor: G,
      newStartPoint: e
    }), D(this, al, a);
    const b = $(this, Nl, td).call(this, X, a, this.themesLoadedData[u].diceAvailable);
    return $(this, Fl, od).call(this, b, a), this.rollCollectionData[a].promise;
  }
  add(X, { theme: u = this.config.theme, themeColor: G = this.config.themeColor, newStartPoint: e = !0 } = {}) {
    const a = nl(this, Vl)._++;
    this.rollCollectionData[a] = new ed({
      id: a,
      notation: X,
      theme: u,
      themeColor: G,
      newStartPoint: e
    }), D(this, al, a);
    const b = $(this, Nl, td).call(this, X, a, this.themesLoadedData[u].diceAvailable);
    return $(this, Fl, od).call(this, b, a), this.rollCollectionData[a].promise;
  }
  reroll(X, { remove: u = !1, hide: G = !1, newStartPoint: e = !0 } = {}) {
    const b = (Array.isArray(X) ? X : [X]).map(({ value: t, ...c }) => c);
    return u === !0 && this.remove(b, { hide: G }), this.add(b, { newStartPoint: e });
  }
  remove(X, { hide: u = !1 } = {}) {
    const G = Array.isArray(X) ? X : [X], e = nl(this, Vl)._++;
    return this.rollCollectionData[e] = new ed({
      id: e,
      notation: X,
      rolls: G
    }), G.map((a) => {
      this.rollDiceData[a.rollId].removeCollectionId = e;
      let b = this.rollDiceData[a.rollId].id;
      F(this, Q).remove({ id: b, rollId: a.rollId }), F(this, _) && F(this, _).postMessage({ action: "removeDie", id: b });
    }), this.rollCollectionData[e].promise;
  }
  // accepts simple notations eg: 4d6
  // accepts array of notations eg: ['4d6','2d10']
  // accepts object {sides:int, qty:int}
  // accepts array of objects eg: [{sides:int, qty:int, mods:[]}]
  createNotationArray(X, u) {
    if (typeof X == "string")
      return Ml(X, u, $(this, Rl, ol).call(this)).diceGroups;
    const G = Array.isArray(X) ? X : [X];
    let e = [];
    const a = (c) => {
      if (c.hasOwnProperty("qty") || (c.qty = 1), c.hasOwnProperty("sides"))
        return c.sides === "100" && (c.sides = 100, c.data = "single"), !0;
      {
        const l = "Roll notation is missing sides";
        throw new Error(l);
      }
    }, b = (c) => {
      c = c.toString();
      let l = c.split(".");
      return l[1] ? l[1] = parseInt(l[1]) + 1 : l[1] = 1, l[0] + "." + l[1];
    }, t = (c) => {
      c.hasOwnProperty("rollId") && this.rollDiceData.hasOwnProperty(c.rollId) && (c.rollId = b(c.rollId)), c.hasOwnProperty("modifier") || (c.modifier = 0);
    };
    return G.forEach((c) => {
      typeof c == "string" ? e.push(...Ml(c, u, $(this, Rl, ol).call(this)).diceGroups) : typeof G == "object" && (t(c), a(c) && e.push(c));
    }), e;
  }
  // parse text die notation such as 2d10+3 => {qty:2, sides:'d10', modifier:3}
  parse(X, u) {
    const G = Ml(X, u, $(this, Rl, ol).call(this)).diceGroups;
    if (G.length !== 1)
      throw new Error("Roll notation expands to multiple dice groups. Use createNotationArray() for compound notation.");
    return G[0];
  }
  getRollResults() {
    const X = Object.entries(this.rollGroupData).map(([G, e]) => {
      const a = $(this, Hl, pd).call(this, G);
      e.value = a.value, e.qty = a.rollsArray.length;
      const b = { ...e };
      return b.rolls = a.rollsArray, b;
    });
    let u = null;
    try {
      u = $(this, Sl, hd).call(this, F(this, al), X);
    } catch {
      u = null;
    }
    return u && Object.defineProperty(X, "finalResult", {
      value: u,
      enumerable: !1,
      configurable: !0
    }), X;
  }
};
Vl = new WeakMap(), sl = new WeakMap(), Jl = new WeakMap(), yl = new WeakMap(), al = new WeakMap(), Q = new WeakMap(), vl = new WeakMap(), fl = new WeakMap(), _ = new WeakMap(), Yl = new WeakMap(), Ll = new WeakMap(), ml = new WeakMap(), Dl = new WeakSet(), nb = async function() {
  D(this, vl, new Promise((u, G) => {
    D(this, fl, u);
  }));
  const X = () => {
    F(this, fl).call(this);
  };
  if (F(this, ml))
    if ("OffscreenCanvas" in window && "transferControlToOffscreen" in this.canvas && this.config.offscreen) {
      const u = await import("./world.offscreen.js").then((G) => G.default);
      D(this, Q, new u({
        canvas: this.canvas,
        options: this.config,
        onInitComplete: X
      }));
    } else {
      this.config.offscreen && (console.warn("This browser does not support OffscreenCanvas. Using standard canvas fallback."), this.config.offscreen = !1);
      const u = await import("./world.onscreen.js").then((G) => G.default);
      D(this, Q, new u({
        canvas: this.canvas,
        options: this.config,
        onInitComplete: X
      }));
    }
  else {
    console.warn("This browser does not support WebGL which is required for 3D rendering. Falling back to random number generator");
    const u = await import("./world.none.js").then((G) => G.default);
    D(this, Q, new u({
      canvas: this.canvas,
      options: this.config,
      onInitComplete: X
    }));
  }
}, Al = new WeakSet(), Vb = function() {
  D(this, _, new zb()), D(this, Yl, new Promise((X, u) => {
    D(this, Ll, X);
  })), F(this, _).onmessage = (X) => {
    switch (X.data.action) {
      case "init-complete":
        F(this, Ll).call(this);
        break;
      case "collision":
        this.onCollision && this.onCollision(X.data.body0Id, X.data.body1Id, X.data.force);
        break;
    }
  }, F(this, _).postMessage({
    action: "init",
    width: this.canvas.clientWidth,
    height: this.canvas.clientHeight,
    options: this.config
  });
}, Ol = new WeakSet(), ab = function() {
  const X = new MessageChannel();
  F(this, Q).connect(X.port1), F(this, _).postMessage({
    action: "connect"
  }, [X.port2]);
}, Rl = new WeakSet(), ol = function() {
  return {
    maxNotationLength: this.config.maxNotationLength,
    maxDiceCount: this.config.maxDiceCount,
    maxSides: this.config.maxSides,
    maxRerollDepth: this.config.maxRerollDepth
  };
}, Nl = new WeakSet(), td = function(X, u, G) {
  const e = this.rollCollectionData[u];
  if (typeof X == "string") {
    const b = Ml(X, G, $(this, Rl, ol).call(this));
    return e.parsedRoll = b, b.diceGroups;
  }
  const a = this.createNotationArray(X, G);
  return e.parsedRoll = {
    originalNotation: Array.isArray(X) ? X.join(",") : "",
    parserNotation: null,
    diceGroups: a,
    parsedTree: null,
    mode: "legacy"
  }, a;
}, Sl = new WeakSet(), hd = function(X, u) {
  const G = this.rollCollectionData[X];
  return G != null && G.parsedRoll ? iX(G.parsedRoll.parsedTree, u, {
    originalNotation: G.parsedRoll.originalNotation || G.notation,
    maxRerollDepth: this.config.maxRerollDepth
  }) : null;
}, Fl = new WeakSet(), od = async function(X, u) {
  this.onBeforeRoll(X);
  const G = this.rollCollectionData[u];
  let e = G.newStartPoint;
  X.forEach(async (a) => {
    var ll, tl, rl, Kl, Cl, wl;
    if (!a.sides)
      throw new Error("Improper dice notation or unable to parse notation");
    let b = a.theme || G.theme || this.config.theme;
    const t = a.themeColor || G.themeColor || this.config.themeColor, c = {}, l = a.groupId !== void 0;
    let n;
    const s = () => this.loadTheme(b);
    await this.loadThemeQueue.push(s);
    let V = this.themesLoadedData[b].meshName, Y = (ll = this.themesLoadedData[b]) == null ? void 0 : ll.diceAvailable, y = this.themesLoadedData[b].diceExtended || {}, N = (rl = (tl = this.themesLoadedData[b]) == null ? void 0 : tl.material) == null ? void 0 : rl.type;
    const J = Object.keys(y);
    if (J && J.includes(a.sides)) {
      b = y[a.sides];
      const ul = () => this.loadTheme(b);
      this.loadThemeQueue.push(ul), V = this.themesLoadedData[b].meshName, Y = (Kl = this.themesLoadedData[b]) == null ? void 0 : Kl.diceAvailable, N = (wl = (Cl = this.themesLoadedData[b]) == null ? void 0 : Cl.material) == null ? void 0 : wl.type;
    }
    let v = "", K;
    N === "color" && (K = kb(t), v = K.r * 0.299 + K.g * 0.587 + K.b * 0.114 > 175 ? "_dark" : "_light");
    for (var T = 0, E = a.qty; T < E; T++) {
      let ul = a.rollId !== void 0 ? a.rollId : nl(this, Jl)._++, zl = a.id !== void 0 ? a.id : nl(this, yl)._++;
      n = l ? a.groupId : F(this, sl);
      const Xl = Number.isInteger(a.sides) ? `d${a.sides}` : a.sides;
      /^d[1-9]{1}[0-9]{0,1}0?$/.test(a.sides) && (a.sides = parseInt(a.sides.replace("d", "")));
      const A = {
        sides: a.sides,
        data: a.data,
        dieType: Xl,
        groupId: n,
        collectionId: G.id,
        rollId: ul,
        id: zl,
        theme: b,
        themeColor: t,
        meshName: V
      };
      if (c[ul] = A, this.rollDiceData[ul] = A, G.rolls.push(this.rollDiceData[ul]), A.sides === "fate" && !Y.includes(Xl) && !J.includes(Xl) || A.sides === "fate" && !F(this, ml)) {
        console.warn(`fate die unavailable in '${b}' theme. Using fallback.`);
        const bl = -1, Gl = 1;
        A.value = lb.range(bl, Gl), F(this, Q).addNonDie(A);
      } else if (this.config.suspendSimulation || !Y.includes(Xl) && !J.includes(Xl) || !F(this, ml)) {
        const bl = F(this, ml) ? this.config.suspendSimulation ? "3D simulation suspended. Using fallback." : `${A.sides} die unavailable in '${b}' theme. Using fallback.` : "This browser does not support webGL. Using random number fallback.";
        console.warn(bl);
        const Gl = Number.isInteger(A.sides) ? A.sides : parseInt(A.sides.replace(/\D/g, ""));
        A.value = lb.range(1, Gl), F(this, Q).addNonDie(A);
      } else {
        let bl;
        if (J.includes(Xl)) {
          const Gl = y[Xl];
          bl = this.themesLoadedData[Gl];
        }
        F(this, Q).add({
          ...A,
          newStartPoint: e,
          theme: (bl == null ? void 0 : bl.systemName) || b,
          meshName: (bl == null ? void 0 : bl.meshName) || V,
          colorSuffix: v
        });
      }
      e = !1;
    }
    l ? Object.assign(this.rollGroupData[n].rolls, c) : (a.rolls = c, a.id = n, this.rollGroupData[n] = a, ++nl(this, sl)._);
  });
}, Hl = new WeakSet(), pd = function(X) {
  const u = this.rollGroupData[X], G = Object.values(u.rolls).map(({ collectionId: a, id: b, meshName: t, ...c }) => c);
  let e = G.reduce((a, b) => {
    const t = isNaN(b.value) ? 0 : b.value;
    return a + t;
  }, 0);
  return e += u.modifier ? u.modifier : 0, { value: e, rollsArray: G };
}, Wl(ql, "createRollApiUrl", Xb);
let cb = ql;
class ed {
  constructor(X) {
    Object.assign(this, X), this.rolls = X.rolls || [], this.completedRolls = 0;
    const u = this;
    this.promise = new Promise((G, e) => {
      u.resolve = G, u.reject = e;
    });
  }
}
export {
  sX as d,
  cb as default
};
//# sourceMappingURL=dice-box.es.js.map
